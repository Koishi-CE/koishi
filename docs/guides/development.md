...
8. **console 类型消费走「源头共享」（2026-09-25 重构）**：浏览器端类型程序以基座 paths 指向各包 lib 产物 d.ts，node 侧的 Services / Events 增强（`declare module "@koishi-ce/console"`）经接线文件 `packages/web/client/console-services.d.ts`（基座 files 下发，副作用导入拉入各包产物）合并生效，client 侧已零手工镜像（原 `shims.d.ts` 骨架与各插件镜像已删）。旧判例「TS7 跨文件 declare module 增强对经 lib 产物 d.ts 的骨架不生效」在 typescript@7.0.2 下经探针实证已不成立（源码文件与 d.ts 内的增强均生效）。两个现役约束：(a) `export *` 转发的实体不能被 augment——增强块会新建同名实体顶替转发实体，增强须挂在实体声明地模块（如 `@koishi-ce/console` 基类包而非 `@koishi-ce/plugin-console`）；(b) 模块名被无 import 的纯声明文件（ambient 声明）占据时遮蔽 paths 解析，纯声明镜像文件必须前置 `import type {}` 转为模块增强。接线纪律由 `check:console-wiring` 对账（含 paths 基座/web 层一致性）；config 的 `PackageProvider` 是全仓唯一 abstract Provider，TS7 对其 d.ts 形态的条件类型匹配静默推出 unknown，增强块已显式标注 `DataService<Dict<PackageProvider.Data>>` 绕开（载荷实体仍是单一事实源）。
