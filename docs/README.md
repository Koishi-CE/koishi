...
> `koishi`（Koishi-CE monorepo）的文档集入口：手册与决策档案的导航页。
...
docs 分两层：**手册**（How to，随代码持续维护）与**档案**（Why / 历史快照，只读）。
...
flowchart LR
  ...
  subgraph D["档案 · 只读"]
    P1["decisions/upgrade-plan.md"]
    P2["decisions/dependency-audit.md"]
  end
  R --- AG
  R --- M
  R --- D
...
| 文档 | 内容速览 | 何时读 |
|---|---|---|
| [DEVELOPMENT.md](DEVELOPMENT.md) | 环境 · 命令 · 门禁 · 构建布局 · 编码约定 · 测试 · 已知坑 | 日常开发、跑门禁前 |
| [ARCHITECTURE.md](ARCHITECTURE.md) | 包清单 · 依赖纪律 · 构建 / 类型 / 测试体系 · 许可证分区 | 改包结构 / 依赖 / 构建链前 |
| [RELEASE.md](RELEASE.md) | changesets · `bun run release` 发布链 · 事故铁律 | 发版前 |
| [UPSTREAM.md](UPSTREAM.md) | 上游基线 · 目录映射表 · port 流程（英文） | 同步上游改动时 |
