...
- others — `plugins/common/rate-limit` from [koishijs/common](https://github.com/koishijs/common), `plugins/infra/memory` (a two-source merge, see the map below), `plugins/infra/server-temp` from [cordiverse/server](https://github.com/cordiverse/server) (upstream deleted `packages/temp` on 2025-03-19 — the line is dead upstream and maintained independently here), `plugins/infra/sqlite` (a three-source merge, see the map below), `plugins/webui/{dataview,theme-vanilla}` from their standalone repos; see `NOTICE` for the full provenance table. `plugins/common/cron` (rewritten on `Bun.cron`, not a direct port) moved out to the [Koishi-CE/services](https://github.com/Koishi-CE/services) monorepo on 2026-09 and is no longer tracked by this page
...
Sub-tree remapping on top of the table above: the client-side UI components (`common/`, `layout/`, `chat/`, `icons/`, `dynamic.vue`, `perms.vue`, `slot.ts`, `link.ts`, `markdown.ts`) were moved from `packages/web/client/src/components/` into `packages/web/components/src/` (client is now a headless runtime core that re-exports the component library). Upstream changes touching webui `packages/client/client/src/components/**` therefore port into `packages/web/components/src/**` — the two table rows above are supersets of upstream's original split.

On top of that remapping, the component library's internal layout was reorganized into domain layers by explicit user authorization (2026-09-25), which exempts it from verbatim upstream file pairing (same regime as the market vendor layer below): `core/` (injection / slot / the markdown sanitize pipeline), `common/` (button / hint / tab / link — internal `k-`-prefixed file names dropped; registration names like `k-button` are unchanged), `form/` (the filter trio, the dynamic / perms schema extensions, and the schemastery-vue dual-track shims renamed to `schemastery-{client,runtime}.ts`), `display/` (comment / markdown view / the image-viewer domain), while `chat/` keeps only the chat-image component and the two image viewers (`viewer.vue` for in-container viewing, `overlay.vue` for fullscreen) share `use-transform.ts` (scale / rotate state) and `toolbar.vue` (the zoom toolbar). Do **not** port upstream component edits file-for-file anymore — locate the counterpart by name semantics and evaluate the change semantically: upstream `k-button.vue` → `common/button.vue`, `image-viewer.vue` → `display/image-viewer/viewer.vue`, `chat/overlay.vue` → `display/image-viewer/overlay.vue`, `chat/utils.ts` → `display/image-viewer/state.ts`, `markdown.ts` → `core/markdown.ts` (pipeline) + `display/markdown.ts` (view), `schemastery-vue-{client,runtime}.ts` → `form/schemastery-{client,runtime}.ts`. The `tooling/upstream-audit` diff is a lead, not a mapping.

Exception to the `src/` rule: the host locale dictionaries that upstream keeps at `packages/client/client/locales/` live at `packages/web/client/locales/` in this repo (outside `src/`, matching the plugin-side `client/locales/` convention); the imports and dev-mode HMR paths in `packages/web/client/src/plugins/{messages,i18n}.ts` reference them via `../../locales/`, so port dictionary edits by that path. The same applies to the host SPA's static assets: upstream `packages/client/app/assets/` maps to `packages/web/app/assets/` (outside `src/`), wired as vite `publicDir: "../assets"` in `packages/web/builder/src/{assemble,index}.ts` and referenced from `src/index.html` as `/logo.png` (copied verbatim to the dist root).

Icon assets have an additional representation change: upstream icons live as template-only `.vue` files under `components/src/icons/{svg,activity}/`, while this repo stores them as plain `.svg` files under `packages/web/components/assets/icons/` (flat names, directory segments joined by hyphens) consumed via `~icons/k/*` virtual modules compiled by `unplugin-icons` (wired in `packages/web/builder/src/icons.ts`; the `.vue` sources were extracted with `tooling/convert-icons-to-svg.ts` and are no longer present). This applies to the webui plugins as well: upstream `plugins/*/*/client/icons/**/*.vue` are centralized into the same directory (plugin-side registration modules keep their `icons.register` calls, importing `~icons/k/<name>` instead of local `.vue` files; the four groups of identical-path duplicates `check` / `trash-can` / `refresh` / `save` were merged into single assets). Those per-plugin registration modules — upstream's `client/icons/index.ts` — now live as `client/icons.ts` in this repo, so port upstream edits to that file accordingly. When porting an upstream change that edits an icon `.vue`, extract the `<template>` body into the corresponding `.svg` by hand instead of copying the file; the registry in `packages/web/components/src/icons/index.ts` maps each icon to its `~icons/k/<name>` import.

The market client vendor layer is exempted from verbatim upstream pairing by explicit user authorization (2026-09-25): the logic vendored from `@koishijs/market` 4.2.10 was originally kept as `client/vendor/market/{utils,icons}.ts` mirroring upstream file-for-file, then reorganized into `plugins/webui/market/client/vendor/{domain,search,sort,icons}.ts` (split by domain; the zh-CN-only local-scope dictionary merged into `client/locales/` under the `market:` root with en-US authored). Do **not** diff these files against upstream `client/utils.ts` line-by-line anymore — port upstream logic changes by semantic evaluation into the matching module. The icon registry lives in `plugins/webui/market/client/vendor/icons.ts` importing `~icons/k/*`; upstream's `plugins/market/client/vendor/market/icons/**` (52 template-only `.vue` files under `misc/` / `outline/` / `solid/`) have no counterpart here, with `misc` names mapped to `market-<name>.svg` and the two stroke variants to `market-outline-<name>.svg` / `market-solid-<name>.svg`. Six misc icons with a same-named semantic counterpart in the main assets (`download` / `file-archive` / `search` / `star-empty` / `star-full` / `tag`) reuse the main asset outright — `tag` / `file-archive` / `search` were verified byte-identical, the rest are FA generation unifications with accepted visual deltas. The `<market-icon>` render layer and its name contract are unchanged. Port upstream icon changes for market by hand-extracting the `<template>` body into the mapped `.svg` (or updating the main asset for the six reused names).
...
1. Clone or fetch the upstream repo at the release line you track — or just run `bun tooling/upstream-audit/index.ts`, which refreshes a shallow-clone cache under `../cache/upstream` (outside this repository; override with `KOISHI_CE_UPSTREAM_CACHE`) and prints a per-directory diff draft.
...
4. Verify with `bun run build` + `bun test` (and `bun run check` when in doubt). Front-end changes additionally need `bun packages/web/builder/src/bin.ts build <plugin-dir>`.
...
3. **Triage every finding** into exactly one class:
   ...
   - **E — conflicts with local intent**: never port. Includes: anything requiring cordis 4 / minato 4 (freeze, see [../decisions/upgrade-plan.md](../decisions/upgrade-plan.md) Phase 5), vendored packages (`plugins/infra/{http,proxy,server}`), frozen shims, the market client fork views and the market locale dictionaries (`plugins/webui/market/locales` — locally re-translated; upstream holds untranslated Chinese placeholders written by the Crowdin bot), Bun-native rewrites (hmr, status agent detection), the explorer glob dependency (`picomatch` used directly instead of upstream's `anymatch` — dependency consolidation, see [../decisions/dependency-audit.md](../decisions/dependency-audit.md) §4.9), and the explorer editor (`CodeMirror 6` instead of upstream's `monaco-editor` — `plugins/webui/explorer/client/{editor.ts,languages.ts,editor.scss}`, see §4.10).
