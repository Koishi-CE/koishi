...
export function useBundleDiff(
	...
	const title = computed(
		() =>
			activeBundle.value?.shortname ||
			activeBundle.value?.package.name ||
			t("bundle.label"),
	);
	/** 展示的合包版本:已解析的 registry 版本优先。 */
	const bundleVersion = computed(
		() =>
			members.resolvedBundleVersion.value ||
			activeBundle.value?.package.version ||
			"",
	);
	/** 清单校验结果(errors 阻断安装,warning 仅提示)。 */
	const validation = computed(() => {
		if (!activeBundle.value || !members.bundle.value)
			return { valid: false, errors: [], warnings: [] };
		return validateBundleManifest(
			activeBundle.value.package.name,
			members.bundle.value as PluginBundleManifest,
			{
	...
	const progressPercent = computed(() =>
		members.members.length
			? Math.round(
					(members.selectedMembers.value.length / members.members.length) * 100,
				)
			: 0,
	);
	...
	const installList = computed(() => {
		...
		return [
			...
			...members.selectedMembers.value.map(
				(member) => `${member.package}@${member.version}`,
			),
		];
	});
	...
	const presetList = computed(() =>
		members.selectedMembers.value
			.filter(
				(member) => member.createConfig && member.usePreset && !member.move,
			)
			.map((member) => member.plugin),
	);
	/** diff"跳过配置"清单:既不建配置也不移动的成员插件键。 */
	const skippedConfigList = computed(() =>
		members.selectedMembers.value
			.filter((member) => !member.createConfig && !member.move)
			.map((member) => member.plugin),
	);
