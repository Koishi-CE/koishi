...
describe('getPackageDescription', () => {
    ...
    const pick = (message?: string | Record<string, string>) =>
        typeof message === 'string' ? message : message?.['zh-CN'] || message?.['en-US']
    ...
    it('manifest 描述优先于 package 描述', () => {
        store.market = {
            data: {
                'pkg-a': {
                    manifest: { description: { 'zh-CN': '首选', 'en-US': 'manifest-en' } },
                    package: { description: { 'en-US': 'package-en' } },
                },
            },
        } as any
        expect(getPackageDescription('pkg-a', pick)).toBe('首选')
        expect(getPackageDescription('pkg-a', (m) => m?.['en-US'])).toBe('manifest-en')
    })

    it('manifest 描述挑选为空时回退 package 描述', () => {
        store.market = {
            data: { 'pkg-a': { manifest: { description: {} }, package: { description: 'fallback' } } },
        } as any
