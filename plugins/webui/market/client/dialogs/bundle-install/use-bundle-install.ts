...
export function useBundleInstall(
	...
	async function confirmInstall() {
		if (!activeBundle.value || !members.bundle.value || installing.value)
			return;
		...
		const runInstall = async (options?: InstallOptions) => {
			...
			try {
				...
				if (result?.code) {
					installProgressState.status = "error";
					reportInstallError(
						t("bundle.messages.exitCode", { code: result.code }),
					);
				...
				const moved = result?.moved?.length
					? t("bundle.messages.moved", { count: result.moved.length })
					: "";
				const skipped = result?.skipped?.length
					? t("bundle.messages.skipped", { count: result.skipped.length })
					: "";
