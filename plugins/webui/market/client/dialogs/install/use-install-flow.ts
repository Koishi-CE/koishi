...
export function useInstallFlow(
	...
	function installDep(
		...
		return install(versions, () =>
			applyInstallResult(target, version, resolved),
		);
	}
	...
	async function applyInstallResult(
		target: string,
		version: string,
		removeConfig: boolean,
	) {
	...
	function requestRemove() {
		...
		const record =
			target &&
			(getBundleRecords(config.value)[target] ||
				createLocalBundleRecord(target));
		if (target && record) {
