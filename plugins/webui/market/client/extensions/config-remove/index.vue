...
const title = computed(() => {
	return target.value?.children
		? t("extensions.actions.removeGroupTitle")
		: t("extensions.actions.removeConfigTitle");
});
...
const content = computed(() => {
	...
	if (!item) return "";
	if (item.children) {
		return t("extensions.messages.removeGroupConfirm", {
			name: item.label || item.path,
		});
	}
	return t("extensions.messages.removeConfigConfirm", {
