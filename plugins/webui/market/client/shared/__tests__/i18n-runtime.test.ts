...
const { ensureLocaleNamespace, installLocaleNamespaceGuard } = await import(
	"../i18n-runtime"
);
...
describe("ensureLocaleNamespace", () => {
	it("词条缺失时 merge 进命名空间并返回 true", () => {
		...
		expect(composer.mergeLocaleMessage).toHaveBeenCalledWith("zh-CN", {
			marketNext: { a: 1 },
		});
	});
	...
	it("词条已完整覆盖预期时跳过 merge 并返回 false", () => {
		const composer = createComposer({
			"zh-CN": { marketNext: { a: 1, nested: { x: true } } },
		});
		const changed = ensureLocaleNamespace(composer, "marketNext", {
	...
	it("叶子值不一致(类型或内容)时视为不完整", () => {
		const composer = createComposer({
			"zh-CN": { marketNext: { a: 1, b: "x" } },
		});
		expect(
			ensureLocaleNamespace(composer, "marketNext", { "zh-CN": { a: 2 } }),
		).toBe(true);
		expect(
			ensureLocaleNamespace(composer, "marketNext", { "zh-CN": { b: ["x"] } }),
		).toBe(true);
	});
	...
	it("现有词条是非对象(数组/null)而预期是对象时视为不完整", () => {
		...
		expect(
			ensureLocaleNamespace(arrayComposer, "marketNext", { "zh-CN": { a: 1 } }),
		).toBe(true);
		const nullComposer = createComposer({ "zh-CN": { marketNext: null } });
		expect(
			ensureLocaleNamespace(nullComposer, "marketNext", { "zh-CN": { a: 1 } }),
		).toBe(true);
	});
	...
	it("locale 整体缺失或 getLocaleMessage 返回非对象时也走 merge", () => {
		const composer = createComposer();
		expect(
			ensureLocaleNamespace(composer, "marketNext", { "en-US": { a: 1 } }),
		).toBe(true);
		expect(
			ensureLocaleNamespace(composer, "marketNext", { "zh-CN": undefined }),
		).toBe(false);
	});
	...
	it("预期叶子为非对象时按引用/值全等比较", () => {
		...
		// 同一引用的数组叶子视为完整
		expect(
			ensureLocaleNamespace(composer, "marketNext", {
				"zh-CN": { list: shared },
			}),
		).toBe(false);
		expect(composer.mergeLocaleMessage).not.toHaveBeenCalled();
		...
		expect(
			ensureLocaleNamespace(composer, "marketNext", { "zh-CN": { list: [1] } }),
		).toBe(true);
	});
	...
	it("多 locale 混合:一个完整一个缺失时仅 merge 缺失的那个", () => {
		...
		expect(composer.mergeLocaleMessage).toHaveBeenCalledWith("en-US", {
			marketNext: { a: 1 },
		});
	});
});
...
describe("installLocaleNamespaceGuard", () => {
	...
	it("guard 补齐过程中再触发 setLocaleMessage 不会递归(applying 标记)", () => {
		...
		installLocaleNamespaceGuard(composer, "marketNext", { "zh-CN": { a: 1 } });
		let mergeCalls = 0;
		composer.mergeLocaleMessage.mockImplementation(
			(locale: string, value: any) => {
				mergeCalls++;
	...
	it("不同 composer 的 guard 相互独立", () => {
		...
		expect(first.mergeLocaleMessage).toHaveBeenCalledWith("zh-CN", {
			marketNext: { a: 1 },
		});
		expect(second.mergeLocaleMessage).toHaveBeenCalledWith("zh-CN", {
			marketNext: { b: 2 },
		});
	});
});
