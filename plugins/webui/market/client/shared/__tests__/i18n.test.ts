 ...
 * @file i18n 装配层的单元测试。
 *
 * vue-i18n 经 vitest alias 指向 @koishi-ce/client,故 useI18n 的桩打在
 * @koishi-ce/client 上。globalComposer 是模块级懒持单例:依赖声明顺序——
...
const { useI18n } = (await import(
	"@koishi-ce/client"
)) as unknown as KoishiClientStub;
const { registerMarketNextI18n, translate, useMarketNextI18n } = await import(
	"../i18n"
);
...
function createComposerStub(initial: Record<string, any> = {}) {
	return {
		locale: { value: "zh-CN" },
		t: vi.fn(
			(key: string, ...args: any[]) => `t:${key}:${JSON.stringify(args)}`,
		),
...
describe("translate", () => {
	...
	it("每次翻译都会 ensure:词条不完整时反复 merge 补齐", () => {
		...
		expect(composer.mergeLocaleMessage.mock.calls.length).toBe(
			callsAfterFirst + 2,
		);
	});
...
describe("registerMarketNextI18n", () => {
	it("注册即安装 guard 并补齐中英两个 locale 的词条", () => {
		const composer = createComposerStub();
		registerMarketNextI18n(createContextStub(composer) as any);
		const locales = composer.mergeLocaleMessage.mock.calls.map(
			([locale]) => locale,
		);
