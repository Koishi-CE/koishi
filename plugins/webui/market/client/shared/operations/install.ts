...
function reportInstallRequestError(error: unknown, messages: InstallMessages) {
	...
	pushInstallLog(
		translate("operations.progress.requestFailed", { detail }),
		"stderr",
	);
	message.error(
		isTimeout
			? (messages.timeoutText ??
					translate("operations.progress.installTimeout"))
...
function beginProgress(partial: {
	title: string;
	selfUpdate: boolean;
	environmentRestore: boolean;
	firstLogs: LogLine[];
}) {
	resetInstallFallbackState();
				...
				? [
						{
							...
							line: translate("operations.progress.selfSubmitted"),
						},
					]
				: []),
		],
	...
	const runInstall = async (options?: InstallOptions) => {
		...
		try {
			...
			if (code) {
				installProgressState.status = "error";
				message.error(
					messages.errorText ?? translate("operations.progress.installError"),
				);
				// 只有真实拿到失败退出码(而非断连)才值得换镜像重试
				if (!state.disconnectedBeforeResponse)
...
export async function applyEnvironmentSnapshot(id: string, selfUpdate = false) {
	beginProgress({
		...
		firstLogs: [
			{
				type: "stdout",
				line: translate("operations.progress.environmentPreparing"),
			},
		],
	});
	...
	const runRestore = async (options?: InstallOptions) => {
		...
		try {
			const task =
				send("market/environment-snapshot-apply", id, options ?? {}) ??
				Promise.resolve(1);
			const code = await Promise.race([task, disconnected]);
			if (state.disconnectedBeforeResponse && !selfUpdate) {
				installProgressState.status = "error";
				pushInstallLog(
					translate("operations.progress.environmentDisconnected"),
					"stderr",
				);
				message.warning(
					translate("operations.progress.environmentDisconnectedShort"),
				);
				return;
			}
			if (code) {
