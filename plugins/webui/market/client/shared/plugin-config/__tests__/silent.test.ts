...
const { store } = await import("@koishi-ce/client");
const { getMarketSilentFilters, getMarketSilentRules } = await import(
	"../silent"
);
...
describe("四组结构化规则转换", () => {
	...
	it("四组规则合并输出且保持组间顺序", () => {
		setupMarketConfig({
			marketSilentStatusRules: [{ target: "preview" }],
			marketSilentDateRules: [
				{ field: "created", relation: "after", date: "2020-01-01" },
			],
