...
export function getMarketSilentFilters(config?: SilentConfig) {
	...
	if (hasOwn(pluginConfig, "marketSilentRules")) {
		return rulesToSilentFilters(
			Array.isArray(pluginConfig.marketSilentRules)
				? pluginConfig.marketSilentRules
				: [],
		).join("\n");
	}
...
export function getMarketSilentRules(config?: SilentConfig) {
	...
	if (hasNewSilentRuleConfig(pluginConfig))
		return structuredSilentRulesToFilters(pluginConfig);
	return [];
}
...
function hasNewSilentRuleConfig(config?: Record<string, any>) {
	return (
		hasConfiguredSilentRules(config?.marketSilentStatusRules) ||
	...
	return rules
		.filter(
			(rule) =>
				...
				rule.field &&
				rule.relation &&
				isDateString(rule.date),
		)
		.map(
			(rule) =>
				`${rule.field}:${rule.relation === "before" ? "<" : ">"}${rule.date}`,
		);
}
...
function ruleToSilentFilter(rule: MarketSilentRule) {
	...
	switch (rule.type) {
		...
		case "bundle":
			return "is:bundle";
		case "created-before":
			return isDateString(date) ? `created:<${date}` : "";
		case "created-after":
			return isDateString(date) ? `created:>${date}` : "";
		case "updated-before":
			return isDateString(date) ? `updated:<${date}` : "";
		case "updated-after":
			return isDateString(date) ? `updated:>${date}` : "";
		case "created-within":
			return isPositiveInteger(days)
				? `created:within:${Math.floor(Number(days))}`
				: "";
		case "updated-within":
			return isPositiveInteger(days)
				? `updated:within:${Math.floor(Number(days))}`
				: "";
		case "custom":
		default:
			return query;
