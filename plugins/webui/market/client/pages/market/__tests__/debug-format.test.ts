...
describe("枚举翻译", () => {
	...
	it("耗时项 key 翻译,未知 key 原样返回", () => {
		expect(formatTimingName("frontendSort", t)).toBe(
			"marketPage.debug.frontendSort",
		);
	...
	it("端点回退原因枚举翻译", () => {
		expect(formatFallbackReason("primary-failed", t)).toBe(
...
describe("压缩比与端点", () => {
	it("压缩比:未压缩显示占位,压缩显示倍率", () => {
		...
		expect(formatCompressionRatio(200, 300, t)).toBe(
			"marketPage.debug.uncompressed",
		);
	...
	it("端点 URL 只显示主机名,解析失败原样返回", () => {
		...
		expect(shortEndpoint("https://registry.npmmirror.com/x")).toBe(
...
describe("阶段概览拼接", () => {
	it("来源/端点/原因/耗时/编码/体积按序拼接,空段丢弃", () => {
		expect(
			formatDebugPhase(
				{ source: "network", endpoint: "https://r.example.com" },
				t,
			),
