...
function lightNode(index: number) {
	const element = nodeLayer.value?.querySelectorAll<SVGCircleElement>(
	...
	effect.addEventListener("finish", () => nodeAnimations.delete(effect), {
		once: true,
	});
}
...
function updateEnhancements() {
	...
	if (frame >= 543 && !completionFired && container.value) {
		...
		completionTimer = setTimeout(
			() => container.value?.classList.remove("is-complete"),
			280,
		);
	}
