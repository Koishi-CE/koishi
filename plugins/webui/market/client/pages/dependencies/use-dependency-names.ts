...
function resolveDiscoveredInput(
	name: string,
...
function shouldIncludeLocalPackage(
	...
	return (
		isUnconfigured(name, ctx, config, configWriter) ||
		isManageableBundle(name, config) ||
		(isPluginPackage(name) &&
			shouldIncludeDiscoveredLocalPlugin(
				resolveDiscoveredInput(name, pkg, configWriter),
			))
	);
}
	...
	watch(
		...
		(value) => {
			void loadMarketObjects(value).catch((error) => {
				console.error(
					"[market-next] failed to load dependency market metadata",
					error,
				);
			});
		},
