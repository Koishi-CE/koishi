...
export function usePackageVisibility(options: {
	...
	const showConfigMeta = computed(
		() =>
			statusClass.value !== "installed" ||
			configText.value !== t("dependencyCard.config.configured"),
	);
	const showSourceMeta = computed(
		() =>
			statusClass.value !== "installed" ||
			sourceText.value !== t("dependencyCard.source.packageJson"),
	);
	...
	const showTargetMeta = computed(() => {
		if (
			...
			state.ignoredUpdate.value
		)
			return true;
		if (statusClass.value === "manual" || statusClass.value === "error")
			return true;
		return (
			!!(state.dep.value || state.local.value) && !state.localDependency.value
		);
	});
	...
	const showBindLocal = computed(() => {
		return (
			!state.pending.value &&
			state.dep.value?.source === "unbound" &&
			state.dep.value?.bound === false
		);
	});

	const showRemoveDependency = computed(() => {
		return (
			(editing.value || statusClass.value !== "installed") &&
			!state.pending.value &&
			!!state.dep.value &&
			!state.dep.value.workspace &&
			!state.dep.value.invalid
	...
	const showCardActions = computed(() => {
		return (
			showVersionControl.value ||
			showQuickUpdate.value ||
			showRestoreUpdate.value ||
			showConfigure.value ||
			showBindLocal.value ||
			showRemoveDependency.value ||
			state.pending.value
		);
	});
