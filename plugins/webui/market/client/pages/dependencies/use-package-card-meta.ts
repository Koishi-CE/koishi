...
export function usePackageCardMeta(
	...
	const configText = computed(() => {
		...
		if (!isPluginPackage(state.name))
			return t("dependencyCard.config.notPlugin");
		if (!getConfigWriter(ctx)) return t("dependencyCard.config.unknown");
		if (!state.local.value)
			return state.pending.value
				? t("dependencyCard.config.pending")
				: t("dependencyCard.config.notLoaded");
		return state.unconfigured.value
			? t("dependencyCard.config.unconfigured")
			: t("dependencyCard.config.configured");
	});

	// 来源文案的优先级映射:bundle/合包自引用/市场源/工作区/本地等来源互斥判定链,顺序即语义
	// fallow-ignore-next-line complexity
	const sourceText = computed(() => {
		if (state.bundleOrigin.value)
			return t("dependencyCard.source.bundle", {
				name:
					state.bundleOrigin.value.label ||
					formatShortname(state.bundleOrigin.value.package),
			});
		if (state.bundleRecord.value) return t("dependencyCard.source.bundleSelf");
		if (state.dep.value?.source)
			return t(`dependencyCard.source.${state.dep.value.source}`);
		if (state.localDependency.value)
			return state.local.value?.workspace
				? t("dependencyCard.source.workspace")
				: t("dependencyCard.source.local");
		if (state.dep.value?.workspace || state.local.value?.workspace)
			return t("dependencyCard.source.workspace");
		if (state.pending.value && !state.dep.value)
			return t("dependencyCard.source.pending");
		if (!state.dep.value && state.local.value)
			return t("dependencyCard.source.local");
	...
	const removeButtonText = computed(() =>
		state.bundleRecord.value
			? t("dependencyCard.actions.uninstallBundle")
			: t("dependencyCard.actions.uninstall"),
	);
	...
	const summaryText = computed(() => {
		...
		const localDescription = (
			state.local.value?.package as { description?: unknown }
		)?.description;
		return (
			tt(state.marketData.value?.manifest?.description) ||
			tt(state.marketData.value?.package?.description) ||
			tt(localDescription as string | Record<string, string> | undefined)
		);
	});
	...
	const editToggleText = computed(() => {
		...
		return state.data.value
			? t("dependencyCard.actions.edit")
			: t("dependencyCard.actions.operate");
	});
