...
export function usePackageCardState(
	...
	const bundleRecord = computed(
		() =>
			getBundleRecords(config.value as any)[props.name] ||
			createLocalBundleRecord(props.name),
	);
	const bundleOrigin = computed(() =>
		findBundleOrigin(props.name, config.value),
	);
	...
	const updateCheckDisabled = computed(() =>
		isUpdateCheckDisabled(props.name, getUpdatePolicy()),
	);
	const ignoredUpdate = computed(
		() =>
			updateCheckDisabled.value ||
			isUpdateIgnored(props.name, getUpdatePolicy()),
	);
	...
	const unconfigured = computed(() => {
		...
		return (
			!!configWriter &&
			!!local.value &&
			isPluginPackage(props.name) &&
			!configWriter.get(props.name)?.length
		);
	});
