...
// 自上游 @koishijs/market 4.2.10（AGPL-3.0）vendor 进本仓。逻辑与上游
// 保持一致，改动仅限：导入名 CE 化（@koishijs/registry → @koishi-ce/registry、
// 类型改 import type）、biome 格式。上游同步按 docs/process/upstream.md
// 手动 diff 本文件与上游 client/utils.ts。
...
export function getUsers(data: SearchObject) {
	...
	if (
		...
		// 上游语义：回退对象允许 name 缺省（username 可为 undefined），
		// 与 User 必有 name 的类型不符，断言仅为类型面收口，运行时不变
...
export function validate(
	...
	if (data.manifest) {
		...
		if (word.startsWith("impl:")) {
			...
			return (
				service.required.includes(name) ||
				service.optional.includes(name)
			);
		} else if (word.startsWith("category:")) {
			return (
				resolveCategory(data.category) === word.slice(9)
			);
		} else if (word.startsWith("email:")) {
			const users = config.users ?? getUsers(data);
			return users.some(
				({ email }) => email === word.slice(6),
			);
