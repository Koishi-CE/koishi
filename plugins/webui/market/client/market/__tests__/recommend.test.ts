...
describe("getRecommendScore 风险乘数与已装降权", () => {
	...
	it("manifest.preview 为 true 乘 0.6", () => {
		expect(score({ manifest: { preview: true } })).toBeCloseTo(
			score() * 0.6,
			8,
		);
	...
	it("config.installed 命中乘 0.18", () => {
		expect(score({}, {}, { installed: () => true })).toBeCloseTo(
			score() * 0.18,
			8,
		);
	});
...
describe("getRecommendScore 六维", () => {
	it("下载量越多人气分越高,缺省下载量按 0 处理", () => {
		...
		expect(negative).toBeLessThanOrEqual(
			score({ downloads: { lastMonth: 0 } as any }),
		);
	});
	...
	it("信任分:verified/portable/包链接逐项加分", () => {
		...
		const linked = score({
			package: { ...basePkg, links: { repository: "https://x" } },
		});
		const homepage = score({
			package: { ...basePkg, links: { homepage: "https://x" } },
		});
		const bugs = score({
			package: { ...basePkg, links: { bugs: "https://x" } },
		});
	...
	it("质量分:描述/分类/关键字/维护者/许可逐项加分,合包被扣", () => {
		...
		const packageDescribed = score({
			package: { ...basePkg, description: "d" },
		});
		const categorized = score({}, { category: "tool" });
		const keyworded = score({
			package: { ...basePkg, keywords: ["a", "b", "c"] },
		});
		const twoKeywords = score({
			package: { ...basePkg, keywords: ["a", "b"] },
		});
		const maintained = score({
			package: { ...basePkg, maintainers: [{ name: "x" }] },
		});
	...
	it("质量分:空白描述与空白多语言对象不加分", () => {
		...
		expect(
			score({ manifest: { description: { "zh-CN": "  ", "en-US": "" } } }),
		).toBe(base);
