...
describe("getSearchIndex", () => {
	...
	it("字符串描述原样归一(不 trim),关键字缺省为空", () => {
		const described = getSearchIndex(
			makeEntry({ manifest: { description: "  Desc  " } }),
		);
	...
	it("rating 为有效数值时保留,否则丢弃", () => {
		...
		expect(
			getSearchIndex(makeEntry({ rating: "x" } as any)).rating,
		).toBeUndefined();
	...
	it("非法日期解析为 NaN 时间戳", () => {
		const index = getSearchIndex(
			makeEntry({ createdAt: "nope", updatedAt: "nope" }),
		);
...
describe("getSearchScoreByIndex", () => {
	...
	it("无 rating 时按更新时间对数衰减,更新时间无效得 0", () => {
		const now = Date.now();
		const fresh = getSearchScoreByIndex(
			makeIndex({ updatedTimestamp: now }),
			[],
			now,
		);
		...
		expect(
			getSearchScoreByIndex(makeIndex({ updatedTimestamp: NaN }), [], now),
		).toBe(0);
	});
	...
	it("任一查询词零相似度直接 0,否则为 rank × 权重和", () => {
		...
		expect(getSearchScoreByIndex(index, ["foo", "foo-bar"])).toBeCloseTo(
			4.5 * 1.5,
			10,
		);
	});
...
describe("getSearchWords", () => {
	it("归一并剔除带冒号的修饰词,再剥包名前缀", () => {
		expect(
			getSearchWords(["Foo Bar", "is:verified", "", "koishi-plugin-webui"]),
		).toEqual(["foo bar", "webui"]);
	});
});
