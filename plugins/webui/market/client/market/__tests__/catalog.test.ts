...
describe("isBundleSearchObject / canInstallBundleSearchObject", () => {
	it("合包命名或 market:package 关键字命中即判定为合包", () => {
		...
		expect(isBundleSearchObject(withPackage("koishi-plugin-pa-suite"))).toBe(
			true,
		);
		expect(
			isBundleSearchObject(withPackage("@scope/koishi-plugin-pa-suite")),
		).toBe(true);
		...
		expect(isBundleSearchObject(withPackage("koishi-plugin-payload"))).toBe(
			false,
		);
		expect(isBundleSearchObject(withPackage("KOISHI-PLUGIN-PA-SUITE"))).toBe(
			false,
	...
	it("canInstallBundleSearchObject 与合包判定等价", () => {
		expect(
			canInstallBundleSearchObject(withPackage("koishi-plugin-pa-suite")),
		).toBe(true);
...
describe("badges", () => {
	it("installed 徽章:未启用安装态或卡片场景隐藏", () => {
		expect(badges.installed!.hidden!({ installed: () => true }, "filter")).toBe(
			false,
		);
		expect(badges.installed!.hidden!({ installed: () => true }, "card")).toBe(
			true,
		);
