...
describe("getSummaryKey", () => {
	...
	it("dataVersion 与 debug hash 拼接成摘要 key", () => {
		...
		expect(
			getSummaryKey({ dataVersion: 3, debug: { hash: "abc" } as any }),
		).toBe("3:abc");
	});
});
...
describe("isSnapshotSuperseded", () => {
	...
	it("store 版本已比响应新时取代", () => {
		expect(
			isSnapshotSuperseded({ ...base, storeVersion: 6, responseVersion: 5 }),
		).toBe(true);
	});

	it("请求期间 key 漂移且响应不是当前 store 那份时取代", () => {
		expect(
			isSnapshotSuperseded({ ...base, currentKey: "6:b", responseKey: "5:a" }),
		).toBe(true);
	});

	it("key 漂移但响应恰好是当前 store 的那份时不取代", () => {
		expect(
			isSnapshotSuperseded({ ...base, currentKey: "6:b", responseKey: "6:b" }),
		).toBe(false);
	});
});
