...
describe("getSortedPrepared", () => {
	...
	it("sort:download / sort:created / sort:updated 切换排序键", () => {
		expect(
			names(getSortedPrepared([alpha, beta, gamma], ["sort:download"])),
		).toEqual([
			"koishi-plugin-beta",
			"koishi-plugin-alpha",
			"koishi-plugin-gamma",
		...
		expect(
			names(getSortedPrepared([alpha, beta, gamma], ["sort:created"])),
		).toEqual([
			"koishi-plugin-beta",
			"koishi-plugin-alpha",
			"koishi-plugin-gamma",
		...
		expect(
			names(getSortedPrepared([alpha, beta, gamma], ["sort:updated"])),
		).toEqual([
			"koishi-plugin-beta",
			"koishi-plugin-alpha",
			"koishi-plugin-gamma",
	...
	it("-asc/-desc 后缀控制方向", () => {
		expect(
			names(getSortedPrepared([alpha, beta, gamma], ["sort:download-asc"])),
		).toEqual([
			"koishi-plugin-gamma",
			"koishi-plugin-alpha",
			"koishi-plugin-beta",
		...
		expect(
			names(getSortedPrepared([alpha, beta, gamma], ["sort:download-desc"])),
		).toEqual([
			"koishi-plugin-beta",
			"koishi-plugin-alpha",
			"koishi-plugin-gamma",
	...
	it("非法排序键回落 default,多个 sort: 词取第一个有效的", () => {
		...
		const sorted = getSortedPrepared(
			[alpha, beta, gamma],
			["sort:bogus", "sort:created-asc"],
		);
	...
	it("sort:recommend 按推荐分排序,-asc 反向", () => {
		expect(
			names(getSortedPrepared([gamma, alpha, beta], ["sort:recommend"])),
		).toEqual([
			"koishi-plugin-beta",
			"koishi-plugin-alpha",
			"koishi-plugin-gamma",
		...
		expect(
			names(getSortedPrepared([gamma, alpha, beta], ["sort:recommend-asc"])),
		).toEqual([
			"koishi-plugin-gamma",
			"koishi-plugin-alpha",
			"koishi-plugin-beta",
	...
	it("recommend 排序受 config.installed 降权影响", () => {
		...
		const config = {
			installed: (data: any) => data.package.name === "koishi-plugin-installed",
		};
		expect(
			names(getSortedPrepared([installed, plain], ["sort:recommend"])),
		).toEqual(["koishi-plugin-installed", "koishi-plugin-plain"]);
		expect(
			names(getSortedPrepared([installed, plain], ["sort:recommend"], config)),
		).toEqual(["koishi-plugin-plain", "koishi-plugin-installed"]);
	});
