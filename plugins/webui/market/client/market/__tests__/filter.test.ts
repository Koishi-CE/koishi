 ...
 *
 * 全链路纯函数(无 @koishi-ce/client 依赖),聚焦 validate 的查询词判定矩阵
...
describe("validateWord / hasFilter / parseSilentFilters", () => {
	...
	it("静音词解析支持多行与中英文分隔符", () => {
		expect(
			parseSilentFilters("is:preview\nupdated:within:30, foo；bar"),
		).toEqual(["is:preview", "updated:within:30", "foo", "bar"]);
...
describe("validate 查询词判定", () => {
	...
	it("is:installed 走 config.installed 回调", () => {
		...
		expect(validate(entry, "is:installed", { installed: () => true })).toBe(
			true,
		);
	...
	it("无 manifest 时 is:/not: 只支持 installed/bundle", () => {
		...
		expect(validate(entry, "not:installed", { installed: () => true })).toBe(
			false,
		);
	});
});
...
describe("getVisible / getFiltered / getSilentFiltered", () => {
	it("getVisible 滤隐藏/弃用条目,show: 前缀放行", () => {
		...
		expect(getVisible([hidden, normal], ["show:hidden"])).toEqual([
			hidden,
			normal,
		]);
	});
	...
	it("getVisible 对弃用条目放行需 show:deprecated,market 缺省安全", () => {
		...
		expect(getVisible([deprecated, normal], ["show:deprecated"])).toEqual([
			deprecated,
			normal,
		]);
	...
	it("getFiltered 多查询词取交集", () => {
		...
		expect(getFiltered([hit, verifiedOnly], ["is:verified"])).toEqual([
			hit,
			verifiedOnly,
		]);
		expect(
			getFiltered([hit, verifiedOnly], ["is:verified", "is:insecure"]),
		).toEqual([hit]);
	});
	...
	it("getSilentFiltered 排除任一命中的条目", () => {
		const verified = makeManifestEntry({ verified: true });
		const plain = makeManifestEntry();
		expect(getSilentFiltered([verified, plain], ["is:verified"])).toEqual([
			plain,
		]);
	});
