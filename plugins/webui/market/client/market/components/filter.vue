...
const activeSort = computed<string[]>(() => {
	...
	if (word.endsWith("-desc")) {
		...
		return [
			supportedSorts.includes(key as (typeof supportedSorts)[number])
				? key
				: "default",
			"desc",
		];
		...
		return [
			supportedSorts.includes(word as (typeof supportedSorts)[number])
				? word
				: "default",
			"desc",
		];
...
const badgeCounts = computed(() => {
	const result: Record<string, number> = Object.fromEntries(
		Object.keys(badges).map((key) => [key, 0]),
	);
...
function isDateToken(word: string, key: DateFilterKey) {
	...
	return (
		word.startsWith(def.prefix + def.operator) ||
...
function readDateFilter(key: DateFilterKey) {
	...
	if (word.startsWith(def.prefix + def.operator)) {
		return normalizeDateValue(
			word.slice(def.prefix.length + def.operator.length),
		);
	}
...
function updateRelativeDateFilterFromEvent(
	key: RelativeDateFilterKey,
	event: Event,
) {
...
function commitRelativeDateFilterFromEvent(
	key: RelativeDateFilterKey,
	event: Event,
) {
	const input = event.target as HTMLInputElement;
...
function clearDateFilters() {
	...
	words.value = words.value.filter((word) => {
		if (
			Object.keys(dateFilterDefs).some((key) =>
				isDateToken(word, key as DateFilterKey),
			)
		)
			return false;
		if (
			Object.keys(relativeDateFilterDefs).some((key) =>
				isRelativeDateToken(word, key as RelativeDateFilterKey),
			)
		)
			return false;
		return true;
