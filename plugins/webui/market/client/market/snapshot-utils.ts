...
export function getSummaryKey(
	value: Partial<MarketProvider.Payload> | undefined,
) {
	if (!value) return "";
	return [value.dataVersion ?? 0, value.debug?.hash ?? ""].join(":");
}

/** 类型守卫:响应是否为"HTTP 传输"形态(数据本体在 url 指向的 gzip 资源里)。 */
export function isMarketSnapshotTransfer(
	value: MarketSnapshotResponse,
): value is MarketSnapshotTransfer {
...
export async function resolveMarketSnapshot(
	value: MarketSnapshotResponse,
): Promise<MarketProvider.Payload> {
...
export function isSnapshotSuperseded(options: {
	...
	if (
		options.storeVersion != null &&
		options.responseVersion != null &&
		options.storeVersion > options.responseVersion
	) {
		return true;
	}
	return !!(
