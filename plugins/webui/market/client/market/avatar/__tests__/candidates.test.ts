...
describe("getUserAvatarCandidates", () => {
	...
	it("主页 url 仅当本身是图片地址才进候选", () => {
		const withImage = getUserAvatarCandidates({
			name: "a",
			email: EMAIL,
			url: "https://a.dev/avatar.jpg",
		} as any);
		expect(withImage.some((item) => item.source === "url")).toBe(true);
		const withPage = getUserAvatarCandidates({
			name: "a",
			email: EMAIL,
			url: "https://a.dev",
		} as any);
	...
	it("有邮箱时生成 gravatar 镜像族与 npm 代理候选,d=404 逐镜像回退", () => {
		...
		expect(candidates.at(-1)?.url).toContain(
	...
	it("自配 gravatar 源最优先且与内置镜像去重", () => {
		const candidates = getUserAvatarCandidates(
			{ name: "a", email: EMAIL } as any,
			"https://s.gravatar.com/avatar/",
		);
		const gravatars = candidates.filter((item) => item.source === "gravatar");
		expect(gravatars.map((item) => item.url)).toHaveLength(
			new Set(gravatars.map((item) => item.url)).size,
		);
		expect(
			candidates[0]?.url.startsWith("https://s.gravatar.com/avatar/"),
		).toBe(true);
	});
	...
	it("非法的 gravatar 自配源被丢弃,不影响内置镜像", () => {
		...
		const gravatars = candidates.filter((item) => item.source === "gravatar");
		expect(gravatars.length).toBeGreaterThanOrEqual(4);
		expect(gravatars.every((item) => item.url.startsWith("https://"))).toBe(
			true,
		);
		// 非 http(s) 协议与根路径形态同样被归一处理
		const ftp = getUserAvatarCandidates(
			{ name: "a", email: EMAIL } as any,
			"ftp://mirror.example/",
		);
		expect(
			ftp
				.filter((item) => item.source === "gravatar")
				.every((item) => item.url.startsWith("https://")),
		).toBe(true);
		const rooted = getUserAvatarCandidates(
			{ name: "a", email: EMAIL } as any,
			"https://mirror.example/",
		);
		expect(rooted[0]?.url.startsWith("https://mirror.example/avatar/")).toBe(
			true,
		);
	});
	...
	it("normalizeAvatarUrl:协议相对地址补全,非法输入原样返回", () => {
		expect(normalizeAvatarUrl("https://EXAMPLE.com/a.png")).toBe(
