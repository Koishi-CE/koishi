 ...
 *
 * mock @koishi-ce/client(fetch 子模块依赖 send),验证聚合导出的每个符号
...
describe("avatar 聚合出口", () => {
	it("re-export 的符号与各子模块逐一同一", async () => {
		...
		expect(avatar.getUserAvatarCandidates).toBe(
			candidates.getUserAvatarCandidates,
		);
		expect(avatar.getCachedAvatarFromCandidates).toBe(
			cache.getCachedAvatarFromCandidates,
		);
