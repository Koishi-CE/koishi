...
function makeMember(
	overrides: Partial<PluginBundleMember> = {},
): PluginBundleMember {
	return {
...
function makeBundle(
	members: PluginBundleMember[] = [makeMember()],
...
describe("validateBundleManifest", () => {
	...
	it("非法包名与非法 semver range 报错", () => {
		const result = validateBundleManifest(
			"koishi-plugin-pa-demo",
			makeBundle([
				makeMember({ package: "not-a-plugin-name", version: "not-semver" }),
			]),
			{ keyword: true },
		);
		expect(result.errors).toContain(
		...
		expect(result.errors).toContain(
			"members[0].version is not a valid semver range",
		);
	});
	...
	it("重复成员报错，同包重复列出给警告", () => {
		...
		expect(result.warnings).toContain(
			"members[1].package is listed more than once",
		);
	});
	...
	it("插件名非小写包形键与潜在冲突给警告", () => {
		...
		expect(result.warnings).toContain(
			"members[1].plugin may conflict with another member",
		);
	});
});
...
describe("scanSensitiveConfig", () => {
	it("递归收集敏感键路径", () => {
		...
		expect(scanSensitiveConfig({ nested: { password: "y" } })).toEqual([
			"nested.password",
		]);
