...
describe("getUpdateCandidates", () => {
	it("返回大于 resolved 的候选并按新到旧排序", () => {
		expect(getUpdateCandidates(VERSIONS, "1.0.0")).toEqual([
			"2.0.0",
			"1.2.0",
			"1.1.0",
		]);
...
describe("isUpdateVersionIgnored", () => {
	...
	it("count 限制被忽略版本上方（更新）的连带忽略数", () => {
		...
		expect(isUpdateVersionIgnored("foo", "1.2.0", candidates, count1)).toBe(
			false,
		);
		expect(isUpdateVersionIgnored("foo", "2.0.0", candidates, count1)).toBe(
			false,
		);
		const count2 = { updateIgnored: { foo: { version: "1.1.0", count: 2 } } };
		expect(isUpdateVersionIgnored("foo", "1.2.0", candidates, count2)).toBe(
			true,
		);
		expect(isUpdateVersionIgnored("foo", "2.0.0", candidates, count2)).toBe(
			false,
		);
		const count3 = { updateIgnored: { foo: { version: "1.1.0", count: 3 } } };
		expect(isUpdateVersionIgnored("foo", "2.0.0", candidates, count3)).toBe(
			true,
		);
	});
	...
	it("until 过期后忽略规则失效", () => {
		const policy = {
			updateIgnored: { foo: { version: "2.0.0", until: 1000 } },
		};
		expect(
			isUpdateVersionIgnored("foo", "2.0.0", ["2.0.0"], policy, 2000),
		).toBe(false);
	});
});
...
describe("getLatestAllowedUpdate", () => {
	it("忽略列表内版本时连带抑制其后的候选（移植行为）", () => {
		...
		expect(getLatestAllowedUpdate("foo", VERSIONS, "1.0.0", policy)).toBe(
			undefined,
		);
	});
