...
export function parseBundleManifest(
	value: unknown,
): PluginBundleManifest | undefined {
...
export function validateBundleManifest(
	...
	if (!bundle.members.length)
		errors.push("koishi.bundle.members must not be empty");
...
function validateBundleMember(
	...
	if (!member.package) errors.push(`${prefix}.package is required`);
		...
		errors.push(
			`${prefix}.package must not reference the bundle package itself`,
		);
	}

	if (!member.plugin) errors.push(`${prefix}.plugin is required`);
	else if (
		!/^(?:@[^/]+\/)?[0-9a-z][0-9a-z-]*(?:\/[0-9a-z][0-9a-z-]*)?$/.test(
			member.plugin,
		)
	) {
		warnings.push(
