...
export type IgnoredUpdates = Record<
	string,
	string | UpdateIgnoreRule | undefined
>;
...
export function normalizeUpdateIgnoreRule(value?: string | UpdateIgnoreRule) {
	...
	if (typeof value === "string")
		return { version: value, count: 1 } as UpdateIgnoreRule;
	return value;
}
...
export function isUpdateCheckDisabled(
	...
	return parseUpdateIgnoredPackages(policy?.updateIgnoredPackages).has(
		normalizeUpdatePackageName(name),
	);
}
...
export function getUpdateCandidates(
	...
	const result = Array.from(new Set(versions)).filter((version) => {
		...
		if (policy?.updateIgnorePrerelease && prerelease(version)?.length)
			return false;
		try {
...
export function getLatestAllowedUpdate(
	...
	return candidates.find(
		(version) =>
			!isUpdateVersionIgnored(name, version, candidates, policy, now),
	);
}
