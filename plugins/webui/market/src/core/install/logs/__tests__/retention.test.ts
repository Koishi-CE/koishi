...
describe("getInstallLogRetention", () => {
	...
	it("installLogRetentionHours 优先且至少 1 小时", () => {
		expect(getInstallLogRetention({ installLogRetentionHours: 2 })).toBe(
			2 * HOUR,
		);
		expect(getInstallLogRetention({ installLogRetentionHours: 0.5 })).toBe(
			HOUR,
		);
	});

	it("legacy installLogRetention 至少 1 小时", () => {
		expect(getInstallLogRetention({ installLogRetention: 3600_000 })).toBe(
			HOUR,
		);
		expect(getInstallLogRetention({ installLogRetention: 5000 })).toBe(HOUR);
		expect(getInstallLogRetention({ installLogRetention: 5 * HOUR })).toBe(
			5 * HOUR,
		);
	});

	it("非法值回退默认", () => {
		expect(getInstallLogRetention({ installLogRetentionHours: -1 })).toBe(
			3 * 24 * HOUR,
		);
		expect(getInstallLogRetention({ installLogRetention: -1 })).toBe(
			3 * 24 * HOUR,
		);
	});
});
