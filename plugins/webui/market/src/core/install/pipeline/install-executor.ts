...
export class InstallExecutor {
	...
	async installLocked(
		...
		const depCache = this.deps.resolver.getDeps({
			background: false,
		}) as Dict<Dependency>;
		return this.runInstallLocked(
			deps,
			forced,
			beforeReload,
			options,
	...
	private async runInstallLocked(
		...
		let logResult:
			| { code?: number | null; failed?: boolean; reason?: string }
			| undefined;
		try {
			const { snapshot, localDeps, needsPackageManager } =
				await prepareAndApplyChanges(
					this.deps,
					deps,
					forced,
					options,
			...
			if (needsPackageManager) {
				const code = await runPackageManagerPhase(
					this.deps,
					deps,
					snapshot,
					options,
