...
export class EnvironmentSnapshotOps {
	...
	async getEnvironmentSnapshots(): Promise<EnvironmentSnapshotSummary[]> {
		...
		return snapshots.map((snapshot) =>
			summarizeEnvironmentSnapshot(snapshot, current.id),
		);
	}
	...
	async getEnvironmentSnapshotPreview(id: string) {
		...
		return {
			...
			unsupportedCount: changes.filter(
				(change) => change.status === "unsupported",
			).length,
		};
	}
	...
	async applyEnvironmentSnapshot(id: string, options: InstallOptions = {}) {
		...
		return this.deps.queue.withLock(`environmentSnapshot=${id}`, async () => {
			...
			const current =
				await this.deps.orchestrator.captureCurrentEnvironmentSnapshot(
					"external",
				);
			...
			return this.deps.orchestrator.installLocked(
				changes,
				true,
				undefined,
				options,
