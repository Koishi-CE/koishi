...
describe("routeScore", () => {
	it("无统计时仅主端点/附加分", () => {
		expect(routeScore(undefined, { isPrimary: true, fastThreshold: 500 })).toBe(
			1,
		);
		expect(
			routeScore(undefined, { isPrimary: false, fastThreshold: 500 }),
		).toBe(0);
		expect(
	...
	it("近期成功加分，过期不加", () => {
		const recent = stats({
			successes: 1,
			failures: 0,
			lastSuccess: NOW - 5 * MINUTE,
		});
		expect(
			routeScore(recent, { isPrimary: false, fastThreshold: 500, now: NOW }),
		).toBeCloseTo(4.75);
		const stale = stats({
			successes: 1,
			failures: 0,
			lastSuccess: NOW - 11 * MINUTE,
		});
		expect(
			routeScore(stale, { isPrimary: false, fastThreshold: 500, now: NOW }),
		).toBeCloseTo(3.25);
	});

	it("连续失败惩罚（上限 5）", () => {
		expect(
			routeScore(stats({ consecutiveFailures: 4 }), {
				isPrimary: false,
				fastThreshold: 500,
			}),
		).toBe(-5);
...
describe("registryFallbackDelay", () => {
	...
	it("高延迟收紧等待", () => {
		expect(registryFallbackDelay(stats({ averageElapsed: 5000 }), 800)).toBe(
			400,
		);
		expect(registryFallbackDelay(stats({ averageElapsed: 3000 }), 800)).toBe(
			600,
		);
		expect(registryFallbackDelay(stats({ averageElapsed: 1000 }), 800)).toBe(
			800,
		);
	});
});
