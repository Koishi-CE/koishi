...
export async function inspectPackageArchive(
		...
		await list({
			...
			onReadEntry(entry) {
				...
				if (
					...
					expandedSize > MAX_ARCHIVE_EXPANDED_SIZE
				) {
	...
	if (!manifestFound)
		throw new Error(
			"本地插件归档中缺少 package/package.json。请使用 npm pack 生成 .tgz。",
		);
	...
	if (!manifest || typeof manifest !== "object")
		throw new Error("本地插件 package.json 无效。");
...
export function createCanonicalLocalPackageFilename(
	name: string,
	version: string,
	hash: string,
) {
	...
	const slug = name
		...
		.replace(/[^a-z0-9@._+-]+/gi, "-")
		.slice(0, 120);
	return createHashedLocalBindingFilename(
		`${slug}-${version}.tgz`,
		hash.slice(0, 12),
	);
}
