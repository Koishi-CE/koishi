...
function makeDeps(
	depCache: Record<string, unknown> = {},
): LocalBindingPrepareDeps {
	return {
		cwd: dir,
...
describe("parseNpmPackOutput", () => {
	...
	it("非数组/空数组/非法 JSON 抛错", () => {
		...
		expect(() => parseNpmPackOutput("[]")).toThrow("invalid npm pack output");
		expect(() => parseNpmPackOutput("not json")).toThrow(
			"invalid npm pack output",
		);
	});

	it("非法文件名抛错", () => {
		expect(() =>
			parseNpmPackOutput('[{"filename":"../evil.tgz","size":1}]'),
		).toThrow("invalid npm pack filename");
		expect(() =>
			parseNpmPackOutput('[{"filename":"foo.tar","size":1}]'),
		).toThrow("invalid npm pack filename");
	...
	it("非法大小抛错", () => {
		expect(() =>
			parseNpmPackOutput('[{"filename":"foo.tgz","size":0}]'),
		).toThrow("invalid npm pack size");
		expect(() =>
			parseNpmPackOutput('[{"filename":"foo.tgz","size":-1}]'),
		).toThrow("invalid npm pack size");
		expect(() =>
			parseNpmPackOutput('[{"filename":"foo.tgz","size":1.5}]'),
		).toThrow("invalid npm pack size");
	...
	it("name/version 为非字符串时归一为 undefined", () => {
		expect(
			parseNpmPackOutput(
				'[{"filename":"foo.tgz","size":10,"name":1,"version":null}]',
			),
		).toEqual({
			name: undefined,
			version: undefined,
...
describe("createLocalBindingRequest", () => {
	it("生成 file:.yarn/local 请求", () => {
		expect(createLocalBindingRequest("foo-1.0.0.tgz")).toBe(
			"file:.yarn/local/foo-1.0.0.tgz",
		);
	});

	it("非法文件名抛错", () => {
		expect(() => createLocalBindingRequest("a/b.tgz")).toThrow(
			"invalid npm pack filename",
		);
	});
});
...
describe("createHashedLocalBindingFilename", () => {
	it("注入内容 hash 前缀", () => {
		...
		).toBe("foo-1.0.0-abcdefabcdef123456789012.tgz");
	});
	...
	it("hash 过短或非法抛错", () => {
		...
		expect(() =>
			createHashedLocalBindingFilename("foo.tgz", "zzzzzzzzzzzz"),
		).toThrow("invalid npm pack hash");
	});
});
...
describe("prepareLocalBinding", () => {
	it("完整链路:校验 → pack → 哈希命名 → 落位 .yarn/local → 清理临时目录", async () => {
		...
		const filename = createHashedLocalBindingFilename(
			`${NAME}-1.0.0.tgz`,
			hash.slice(0, 12),
		);
		...
		await expect(
			fsp.readFile(join(dir, ".yarn", "local", filename), "utf8"),
		).resolves.toBe(PACKED_CONTENT);
		// 临时打包目录被清理
		const leftovers = (await readdir(join(dir, ".yarn", "local"))).filter(
			(name) => name.startsWith(".market-next-pack-"),
		);
	...
	it("非插件名 / 不在依赖表中时拒绝", async () => {
		await makeHost();
		const deps = makeDeps(healthyDepCache());
		await expect(prepareLocalBinding(deps, "foo")).rejects.toThrow(
			"只能绑定当前 package.json",
		);
		await expect(
			prepareLocalBinding(deps, "koishi-plugin-other"),
		).rejects.toThrow("只能绑定当前 package.json");
	});
	...
	it("同名归档内容不一致时拒绝覆盖", async () => {
		...
		await writeFile(
			join(dir, ".yarn", "local", filename),
			"Xacked-archive-content",
		);
		const deps = makeDeps(healthyDepCache());
		await expect(prepareLocalBinding(deps, NAME)).rejects.toThrow(
