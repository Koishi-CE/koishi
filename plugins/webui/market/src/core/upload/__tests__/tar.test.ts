...
describe("inspectPackageArchive", () => {
	...
	it("根之外的 package.json 不被认作 manifest", async () => {
		const path = await pack([
			{ name: "package/nested/package.json", content: DEMO_MANIFEST },
		]);
		await expect(inspectPackageArchive(path)).rejects.toThrow(
			"缺少 package/package.json",
		);
	});
	...
	it("package.json 不是合法 JSON 时报错", async () => {
		const path = await pack([
			{ name: "package/package.json", content: "{oops" },
		]);
		await expect(inspectPackageArchive(path)).rejects.toThrow(
			"不是有效的 JSON",
		);
	...
	it("package.json 解析结果非对象时报错", async () => {
		const nullPath = await pack([
			{ name: "package/package.json", content: "null" },
		]);
		await expect(inspectPackageArchive(nullPath)).rejects.toThrow(
			"本地插件 package.json 无效",
		);
		const numberPath = await pack([
			{ name: "package/package.json", content: "42" },
		]);
	...
	it("海量条目触发数量上限防护", async () => {
		...
		const specs: TarEntrySpec[] = [
			{ name: "package/package.json", content: DEMO_MANIFEST },
		];
		for (let i = 0; i < 8192; i++)
			specs.push({ name: `package/f${i}`, content: "" });
...
describe("createCanonicalLocalPackageFilename", () => {
	...
	it("超长 slug 截断到 120 字符", () => {
		...
		const filename = createCanonicalLocalPackageFilename(
			name,
			"1.0.0",
			"a".repeat(12),
		);
...
describe("readFileHash", () => {
	...
	it("返回内容的 sha256 hex", async () => {
		...
		expect(await readFileHash(path)).toBe(
	...
	it("目标是目录时报错", async () => {
		...
		await mkdir(path);
		await expect(readFileHash(path)).rejects.toThrow(
			"本地插件归档目标不是文件",
		);
	});
});
