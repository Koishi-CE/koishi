...
describe("validateUploadFilename", () => {
	...
	it("拒绝非字符串", () => {
		expect(() => validateUploadFilename(undefined)).toThrow(
			"请选择 npm pack 生成的 .tgz 文件",
		);
		expect(() => validateUploadFilename(123)).toThrow(
			"请选择 npm pack 生成的 .tgz 文件",
		);
	});

	it("拒绝带路径分隔的文件名", () => {
		expect(() => validateUploadFilename("a/b.tgz")).toThrow(
			"请选择 npm pack 生成的 .tgz 文件",
		);
...
describe("decodeBase64Chunk", () => {
	it("解码合法分块", () => {
		const buffer = decodeBase64Chunk(
			Buffer.from("market-next").toString("base64"),
		);
	...
	it("拒绝非字符串与非法字符", () => {
		expect(() => decodeBase64Chunk(undefined)).toThrow(
			"本地插件上传分块编码无效",
		);
		expect(() => decodeBase64Chunk(123)).toThrow("本地插件上传分块编码无效");
		expect(() => decodeBase64Chunk("!!!not-base64")).toThrow(
			"本地插件上传分块编码无效",
		);
		// 含 url-safe 字母表(-)同样拒绝
		expect(() => decodeBase64Chunk("ab-cd")).toThrow(
			"本地插件上传分块编码无效",
		);
	});

	it("拒绝长度非 4 倍数的编码", () => {
		expect(() => decodeBase64Chunk("abcde")).toThrow(
			"本地插件上传分块编码无效",
		);
	});
	...
	it("拒绝超过单块上限的编码", () => {
		...
		const oversized = Buffer.alloc(LOCAL_UPLOAD_CHUNK_SIZE + 64).toString(
			"base64",
		);
		expect(oversized.length).toBeGreaterThan(limit);
		expect(() => decodeBase64Chunk(oversized)).toThrow(
			"本地插件上传分块编码无效",
		);
	});
...
describe("sweepTemporaryUploads", () => {
	...
	it("目录路径指向文件时(非 ENOENT 错误)向上抛出", async () => {
		...
		await expect(
			sweepTemporaryUploads(blocked, new Set(), Date.now(), 1000),
		).rejects.toThrow();
	});
...
describe("placeUploadArchive", () => {
	...
	it("目标不存在时 rename 落位并返回目标路径", async () => {
		...
		const target = await placeUploadArchive(
			dir,
			sessionPath,
			"demo-1.0.0.tgz",
			hash,
		...
		await expect(fsp.stat(sessionPath)).rejects.toMatchObject({
			code: "ENOENT",
		});
	});
	...
	it("目标已存在但内容不一致时拒绝覆盖", async () => {
		...
		await expect(
			placeUploadArchive(dir, sessionPath, "demo-1.0.0.tgz", hash),
		).rejects.toThrow("同名本地插件归档已存在，但文件内容不一致。");
	});
