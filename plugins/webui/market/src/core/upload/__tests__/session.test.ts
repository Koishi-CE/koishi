...
describe("LocalPackageUploadStore", () => {
	...
	it("start 拒绝非法文件名", async () => {
		await expect(
			store.start({ filename: "a/b.tgz", size: 10 }),
		).rejects.toThrow("请选择 npm pack 生成的 .tgz 文件。");
		await expect(
			store.start({ filename: "demo.tar", size: 10 }),
		).rejects.toThrow("请选择 npm pack 生成的 .tgz 文件。");
	});

	it("start 拒绝非法大小", async () => {
		await expect(
			store.start({ filename: "demo.tgz", size: 0 }),
		).rejects.toThrow("本地插件归档大小必须在 1 B 到");
		await expect(
			store.start({ filename: "demo.tgz", size: 64 * 1024 * 1024 + 1 }),
		).rejects.toThrow("本地插件归档大小必须在 1 B 到");
		await expect(
			store.start({ filename: "demo.tgz", size: 1.5 }),
		).rejects.toThrow("本地插件归档大小必须在 1 B 到");
	});
	...
	it("finish 在未传完时拒绝", async () => {
		...
		await expect(store.finish({ uploadId: started.uploadId })).rejects.toThrow(
			"尚未上传完成",
		);
	});
	...
	it("commit 在未校验时拒绝", async () => {
		...
		await expect(store.commit(started.uploadId)).rejects.toThrow(
			"请先完成本地插件归档校验",
		);
	});
	...
	it("非法 uploadId 拒绝", async () => {
		...
		await expect(store.finish({ uploadId: "not-a-uuid" })).rejects.toThrow(
			"上传会话无效",
		);
	});
	...
	it("pruneExpired 清理失败仅告警不抛出", async () => {
		...
		await expect(
			store.pruneExpired(Date.now() + 15 * 60 * 1000 + 1),
		).resolves.toBeUndefined();
		expect(warn).toHaveBeenCalledWith(
			expect.stringContaining(started.uploadId),
		);
	});
	...
	it("append 请求缺省/非整数序号/空块/超限块均拒绝", async () => {
		const started = await store.start({ filename: "demo.tgz", size: 10 });
		await expect(store.append(undefined as never)).rejects.toThrow(
			"本地插件上传会话无效",
		);
		await expect(
	...
	it("finish 校验失败即销毁会话,不可继续操作", async () => {
		vi.mocked(inspectPackageArchive).mockRejectedValueOnce(
	...
	it("commit 真实落盘到 .yarn/local 并销毁会话", async () => {
		...
		await expect(fsp.readFile(target)).resolves.toEqual(
