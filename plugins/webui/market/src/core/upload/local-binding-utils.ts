...
export function parseNpmPackOutput(output: string): LocalBindingPackResult {
	...
	if (
		...
		size <= 0 ||
		size > MAX_LOCAL_BINDING_PACK_SIZE
	) {
 ...
 */
export function createHashedLocalBindingFilename(
	filename: string,
	hash: string,
) {
	const safeFilename = validatePackFilename(filename);
	if (!/^[a-f0-9]{12,64}$/i.test(hash))
		throw new Error("invalid npm pack hash");
...
export function assertBindableDependency(
	...
	if (
		...
		!Object.hasOwn(packageSnapshot.dependencies, name)
	) {
