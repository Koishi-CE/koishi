...
interface MarketFetchHostSource {
	...
	readonly cache: Pick<
		MarketDiskCache,
		"entries" | "loadEntryResult" | "conditionalHeaders"
	>;
}

/** 把 MarketIndexSource 的公开状态适配为索引竞速所需的 FetchIndexDeps。 */
export function buildMarketFetchDeps(
	source: MarketFetchHostSource,
	deps: MarketSourceDeps,
) {
	return {
		...
		conditionalHeaders: (endpoint: string) =>
			source.cache.conditionalHeaders(endpoint),
		log: deps.log,
	};
}
...
export async function fetchMarketIndex(
	...
	try {
		...
		if (
			!rescueEndpoints.length ||
			deps.scope.isStale(serial) ||
		...
		const result = await raceIndexFrom(deps, serial, rescueEndpoints, {
			rescue: true,
		});
