...
export async function collectMarketIndex(
	source: MarketCollectSource,
): Promise<undefined> {
	...
	source.updateDebugInfo(
		performanceFrom(result, source.scanner.total),
		"initial",
	);
...
async function analyzeLegacy(source: MarketCollectSource) {
	...
	await source.scanner.analyze({
		...
		onFailure: (name: string, reason: unknown) => {
			...
			source.log.debug(
				`failed to analyze package ${name}: ${formatError(reason)}`,
			);
		},
		...
		onSuccess: (object: SearchObject) => {
			source.fullCache[object.package.name] = source.tempCache[
				object.package.name
			] = object;
		},
