...
export function marketRouteScore(
	endpoint: string,
...
export function getEndpointCandidates(config: MarketEndpointConfig) {
	return [
		config.endpoint,
		...(config.autoRoute === false ? [] : FALLBACK_ENDPOINTS),
	].filter(
...
export function getRaceEndpoints(
	...
	const originalIndex = new Map(
		fallbacks.map((endpoint, index) => [endpoint, index]),
	);
	return [
		primary,
		...available.sort((a, b) => {
...
export function getRescueEndpoints(
	activeEndpoints: string[],
	context: MarketScoreContext,
) {
	if (context.config.autoRoute === false) return [];
	const active = new Set(activeEndpoints);
	return getEndpointCandidates(context.config).filter(
		(endpoint) => !active.has(endpoint),
	);
}
