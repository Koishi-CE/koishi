...
describe("getEndpointCandidates", () => {
	...
	it("自定义主端点在首位", () => {
		const candidates = getEndpointCandidates({
			endpoint: "https://custom.example/index.json",
		});
...
describe("marketRouteScore", () => {
	...
	it("磁盘缓存新鲜度加分（相对无缓存）", () => {
		...
		expect(
			marketRouteScore("https://a", fresh) -
				marketRouteScore("https://a", plain),
		).toBeCloseTo(1.5);
		...
		expect(
			marketRouteScore("https://a", stale) -
				marketRouteScore("https://a", plain),
		).toBeCloseTo(0.5);
	});
