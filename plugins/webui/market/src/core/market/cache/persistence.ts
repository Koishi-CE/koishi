...
export function buildCacheMeta(
	...
	return {
		...
		contentEncoding: pickStat(
			result.contentEncoding,
			previous?.contentEncoding,
		),
	};
}
...
function getFetchedAt(
	...
	return (
		result.cachedAt ?? cached?.fetchedAt ?? previous?.fetchedAt ?? Date.now()
	);
}
...
export function restoreRouteStats(
	...
	for (const [endpoint, stats] of Object.entries(routeStats)) {
		...
		const hasRecentSuccess =
			stats.lastSuccess && Date.now() - stats.lastSuccess < DAY;
		target[endpoint] = {
			score: hasRecentSuccess
				? clamp(stats.score, -1, 3)
				: clamp(stats.score, -4, 3),
			successes: 0,
			failures: 0,
