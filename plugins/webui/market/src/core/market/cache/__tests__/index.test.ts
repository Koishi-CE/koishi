...
async function createCache(content?: unknown) {
	...
	if (content !== undefined)
		await fsp.writeFile(cacheFile, JSON.stringify(content));
	const cache = new MarketDiskCache({
		...
		scoreContext: () =>
			({ config: { endpoint: "https://primary.example" } }) as never,
		endpointCandidates: () => [
...
describe("MarketDiskCache.load", () => {
	...
	it("恢复 split cache 条目和近期路由统计", async () => {
		...
		expect(loaded.applied?.result.objects[0]).toMatchObject({
			name: "primary",
		});
	});
});
