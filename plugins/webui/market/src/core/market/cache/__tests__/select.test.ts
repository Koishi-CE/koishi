...
function makeContext(
	overrides: Partial<MarketScoreContext> = {},
): MarketScoreContext {
	return {
...
describe("cacheEntryScore", () => {
	...
	it("新鲜度分档:半天内 +3、3 天内 +1、更旧 -1", () => {
		...
		const fresh = cacheEntryScore(
			makeEntry("https://mirror.example", NOW - 6 * HOUR),
			context,
		);
		const mid = cacheEntryScore(
			makeEntry("https://mirror.example", NOW - 2 * DAY),
			context,
		);
		const old = cacheEntryScore(
			makeEntry("https://mirror.example", NOW - 10 * DAY),
			context,
		);
	...
	it("用户首选端点在路由评分与首选加分上均占优", () => {
		...
		const mirror = cacheEntryScore(
			makeEntry("https://mirror.example", NOW - HOUR),
			context,
		);
	...
	it("fetchedAt 非有限值按最旧档计分", () => {
		const context = makeContext();
		const broken = cacheEntryScore(
			makeEntry("https://mirror.example", Number.NaN),
			context,
		);
...
describe("buildCacheCandidates", () => {
	...
	it("排除主端点与无索引引用的条目,列表外端点不参与", () => {
		...
		const entries: Record<string, CacheEntry> = {
			...
			"https://outside.example": makeEntry(
...
describe("pruneCacheEntries", () => {
	...
	it("过滤无索引引用与超过 30 天 TTL 的条目", () => {
		const entries: Record<string, CacheEntry> = {
			kept: makeEntry("https://kept.example", NOW - HOUR, {
				file: "kept.json",
			}),
	...
	it("lastUsed 与用户首选端点优先,超出上限截取 3 条", () => {
		const entries: Record<string, CacheEntry> = {
			used: makeEntry("https://used.example", NOW - 4 * HOUR, {
				file: "used.json",
			}),
	...
	it("返回以 endpoint 为键的新字典", () => {
		const entry = makeEntry("https://a.example", NOW - HOUR, {
			file: "a.json",
		});
		const pruned = pruneCacheEntries(
