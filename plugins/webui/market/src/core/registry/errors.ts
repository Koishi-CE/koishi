...
export function formatRegistryError(
	...
	const attached = (error as { marketNextReason?: RegistryReason } | undefined)
		?.marketNextReason;
	if (attached) {
		return {
			reason: attached,
	...
	if (isHttpError(error)) {
		...
		if (status)
			return { reason: "http", error: `npm 元数据请求失败，HTTP ${status}。` };
	}
	if (/timeout|ETIMEDOUT|ECONNABORTED/i.test(message)) {
		return { reason: "timeout", error: "npm 元数据请求超时。" };
	}
	if (
		/ENOTFOUND|ECONNRESET|ECONNREFUSED|EAI_AGAIN|fetch failed|network/i.test(
			message,
		)
	) {
...
export function attachRegistryAttemptReasons(
	error: unknown,
