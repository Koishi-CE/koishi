...
export async function fetchRegistryByRoute(
	...
	return raceEndpoints<Registry>({
		...
		fetch: (endpoint, signal) =>
			fetchRegistryEndpoint(deps, name, endpoint, serial, signal),
		onAttempt: (endpoint) => {
			attempts++;
		...
		onFailure: (endpoint, error) => {
			...
			if (shouldPenalizeRegistryRoute(reason))
				deps.recordRouteFailure(endpoint, reason);
		},
...
async function fetchRegistryEndpoint(
	...
	deps.log.debug(
		`fetch npm registry endpoint: package=${name}, endpoint=${endpoint}`,
	);
	const registry = await deps
		.httpFactory(endpoint)
		.get(`/${name}`, signal ? { signal } : undefined);
	if (deps.scope.isStale(serial))
		throw new Error("npm registry route probe stale");
