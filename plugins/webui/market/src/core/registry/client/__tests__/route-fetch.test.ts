...
describe("fetchRegistryByRoute", () => {
	...
	it("单端点网络失败:抛错、按 network 惩罚、归因挂到异常", async () => {
		...
		expect(deps.recordRouteFailure).toHaveBeenCalledWith(
			PRIMARY_ENDPOINT,
			"network",
		);
		const attached = (error as { marketNextReasons?: string[] })
			.marketNextReasons;
	...
	it("全部端点失败:抛最后错误并聚合各端点归因(404 不惩罚)", async () => {
		...
		expect(
			(third as { marketNextReasons?: string[] }).marketNextReasons,
		).toEqual(["not-found", "network", "timeout"]);
	});

	it("竞速过程日志经 debug 通道输出", async () => {
		const deps = makeRouteDeps();
		await fetchRegistryByRoute(
			deps,
			"pkg",
			[PRIMARY_ENDPOINT],
			deps.scope.current,
		...
		expect(deps.log.debug).toHaveBeenCalledWith(
			expect.stringContaining("npm registry"),
		);
	});
});
