...
function attemptOf(
	overrides: Partial<RaceAttempt<Registry>> = {},
): RaceAttempt<Registry> {
	return {
...
describe("fetchRegistryWithRetry", () => {
	...
	it("竞速胜出端点与当前元数据端点不同时切换并输出路由日志", async () => {
		...
		const result = await fetchRegistryWithRetry("pkg", 7, host);
		expect(result).toBeDefined();
		expect(host.setMetadataEndpoint).toHaveBeenCalledWith(
			"https://mirror.example.com",
		);
		expect(host.log.info).toHaveBeenCalledWith(
			expect.stringContaining("route selected"),
		);
	});
	...
	it("胜出端点无降级原因时路由日志记为 same-priority", async () => {
		...
		expect(host.log.info).toHaveBeenCalledWith(
			expect.stringContaining("reason=same-priority"),
		);
	});
	...
	it("首轮失败后线性退避重试,次轮成功返回负载", async () => {
		...
		const { host } = makeFetchHost({
			fetchByRoute:
				fetchByRoute as unknown as RegistryFetchHost["fetchByRoute"],
		});
		const result = await fetchRegistryWithRetry("pkg", 7, host);
		expect(result).toBeDefined();
	...
	it("全部轮次失败:合并归因上报并以属性形式挂到异常上", async () => {
		...
		await expect(fetchRegistryWithRetry("pkg", 7, host)).rejects.toThrow(
			"fetch failed",
		);
		const calls = vi.mocked(host.statusSink).mock.calls;
		...
		expect((error as { marketNextReason?: string }).marketNextReason).toBe(
			"network",
		);
		expect(
			(error as { marketNextReasons?: string[] }).marketNextReasons,
		).toContain("network");
	});
	...
	it("多路由归因含非 not-found 原因时最终 reason 取首个非 404", async () => {
		...
		expect(vi.mocked(host.statusSink).mock.calls.at(-1)?.[1]?.reason).toBe(
			"timeout",
		);
	});
	...
	it("探测完成后请求已过期:返回 undefined 且不再竞速", async () => {
		...
		await expect(
			fetchRegistryWithRetry("pkg", 8, host),
	...
	it("竞速成功后请求过期:返回 undefined 且不上报成功", async () => {
		...
		await expect(
			fetchRegistryWithRetry("pkg", 7, host),
		).resolves.toBeUndefined();
		const statuses = vi
			.mocked(host.statusSink)
			.mock.calls.map((call) => call[1]);
