...
function statsBookOf(map: Record<string, Partial<RouteStats>>): RouteStatsBook {
	return {
		get: (endpoint: string) => map[endpoint],
	} as unknown as RouteStatsBook;
}

describe("sortRouteProbeEndpoints", () => {
	it("autoRoute 关闭时只返回主端点", () => {
		expect(
			sortRouteProbeEndpoints({ autoRoute: false }, "https://a", () => 0),
		).toEqual(["https://a"]);
	});
...
describe("preferredMetadataEndpoint", () => {
	...
	function makeOptions(
		overrides: Partial<Parameters<typeof preferredMetadataEndpoint>[0]> = {},
	) {
		const options = {
			endpoint,
	...
	it("连续失败不足 2 次时保持原选择", () => {
		const options = makeOptions({
			stats: statsBookOf({ "https://mirror": { failures: 1 } }),
		});
...
describe("registryRouteScores", () => {
	it("主端点带错峰延迟,镜像延迟为 undefined,并展开统计明细", () => {
		...
		const npmjsRow = rows.find(
			(row) => row.endpoint === "https://registry.npmjs.org",
		)!;
...
describe("installFallbackCandidate", () => {
	function makeOptions(
		overrides: Partial<Parameters<typeof installFallbackCandidate>[0]> = {},
	) {
		return {
			config: {},
			endpoint: "https://a",
