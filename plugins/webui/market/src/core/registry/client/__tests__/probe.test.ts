...
function attemptOf(
	overrides: Partial<RaceAttempt<ReturnType<typeof makeRegistryPayload>>> = {},
) {
	return {
...
describe("RouteProbe.ensure", () => {
	...
	it("探测成功:主端点胜出时记录结果并输出 primary 日志", async () => {
		...
		expect(log.info).toHaveBeenCalledWith(
			expect.stringContaining("primary selected"),
		);
		expect(log.debug).toHaveBeenCalledWith(
			expect.stringContaining("route scores"),
		);
	...
	it("探测成功:镜像胜出时输出 fallback 日志并记录原因", async () => {
		...
		expect(log.info).toHaveBeenCalledWith(
			expect.stringContaining("fallback selected"),
		);
	});
	...
	it("单飞去重:并发 ensure 共享同一次探测", async () => {
		...
		const fetchByRoute = vi.fn(
			() =>
				new Promise<RaceAttempt<ReturnType<typeof makeRegistryPayload>>>(
					(resolve) => {
	...
	it("探测失败:仅告警不抛出、不落结果", async () => {
		...
		await expect(
			probe.ensure("pkg", ENDPOINTS, 0, fetchByRoute),
