...
describe("RegistryClient.resetEndpoint", () => {
	...
	it("端点真的变化时重置路由学习统计,首次复位与同端点复位不重置", async () => {
		...
		expect(deps.log.info).toHaveBeenCalledWith(
			expect.stringContaining("endpoint changed"),
		);
	});
});
...
describe("RegistryClient 统计持久化与评分", () => {
	...
	it("restoreStats 从持久化视图恢复学习数据", async () => {
		...
		const store: RegistryStatsStore = {
			...
			savedAt: NOW - 1000,
			stats: {
				"https://x": { score: 1, successes: 2, failures: 0, lastSuccess: NOW },
			},
		};
		await client.restoreStats(store);
		expect(deps.stats.get("https://x")).toMatchObject({
			score: 1,
			successes: 2,
		});
	});
	...
	it("getRouteScores 输出全部候选且仅主端点带错峰延迟", async () => {
		...
		expect(rows.slice(1).every((row) => row.fallbackDelay === undefined)).toBe(
			true,
		);
	});
...
describe("RegistryClient.getRegistry 全链路", () => {
	it("首个请求:探测竞速主端点胜出,负载直接复用(仅一次 HTTP)", async () => {
		...
		expect(client.probeResult).toMatchObject({
			name: "pkg",
			endpoint: PRIMARY_ENDPOINT,
		});
		// 探测胜出被记录为一次成功路由
	...
	it("全部端点 404:探测仅告警,最终失败归因 not-found 且不惩罚镜像", async () => {
		...
		expect(deps.log.warn).toHaveBeenCalledWith(
			expect.stringContaining("route probe failed"),
		);
	...
	it("formatError 透传注入的 isHttpError 判定", () => {
		...
		expect(client.formatError(new Error("request timeout")).reason).toBe(
			"timeout",
		);
	});
});
