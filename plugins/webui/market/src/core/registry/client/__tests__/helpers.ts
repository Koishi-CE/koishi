...
export function makeRegistryPayload(
	...
	for (const item of versions) {
		map[item.version] = remotePackage(
			item.version,
...
export function makeRouteDeps(
	overrides: Partial<RegistryRouteDeps> = {},
): RegistryRouteDeps {
	return {
...
export function makeClientDeps(
	overrides: Partial<RegistryClientDeps> = {},
): RegistryClientDeps {
	return {
		httpFactory: makeHttpFactory({ [PRIMARY_ENDPOINT]: makeRegistryPayload() }),
