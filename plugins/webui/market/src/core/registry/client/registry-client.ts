...
export class RegistryClient implements RegistryFetchHost {
	...
	constructor(deps: RegistryClientDeps, config: RegistryClientConfig = {}) {
		...
		this.probe = new RouteProbe({
			...
			log: deps.log,
			scoresSummary: () =>
				JSON.stringify(this.getRouteScores().map((item) => item.endpoint)),
		});
		...
		this.routeDeps = {
			...
			recordRouteFailure: (endpoint, reason) =>
				this.recordRouteFailure(endpoint, reason),
		};
	}
	...
	async resetEndpoint() {
		const endpoint =
			this.config.endpoint || (await this.deps.defaultEndpoint());
	...
	async ensureMetadataEndpoint(name: string, serial: number) {
		const endpoints = sortRouteProbeEndpoints(
			this.config,
			this.endpoint,
			(item) => this.getRouteScore(item),
		);
		await this.probe.ensure(name, endpoints, serial, (n, e, s) =>
			this.fetchByRoute(n, e, s),
		);
	}
