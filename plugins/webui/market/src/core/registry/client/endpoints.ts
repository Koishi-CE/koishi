...
function registryEndpointCandidates(
	config: RegistryClientConfig,
	endpoint: string,
) {
	return [
		endpoint,
		...(config.autoRoute === false ? [] : REGISTRY_FALLBACK_ENDPOINTS),
	].filter(
...
export function registryRouteScores(options: {
	...
	return registryEndpointCandidates(options.config, options.endpoint).map(
		(item) => ({
			endpoint: item,
			score: options.score(item),
			// 只对主端点给出错峰延迟(其余端点的延迟由竞速器内部计算)
			fallbackDelay:
				item === options.endpoint ? options.fallbackDelay(item) : undefined,
...
export function installFallbackCandidate(options: {
	...
	const candidates = registryEndpointCandidates(config, endpoint)
		...
		.sort((a, b) => {
			...
			const successDelta =
				(b.stats?.lastSuccess ?? 0) - (a.stats?.lastSuccess ?? 0);
	...
	return {
		...
		reason: candidate.stats?.lastSuccess
			? "最近可用的备用 npm 源"
			: "备用 npm 源",
	};
}
