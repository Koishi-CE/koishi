...
describe("formatRegistryError", () => {
	...
	it("按消息分类超时/网络/格式/未知", () => {
		expect(
			formatRegistryError(new Error("request timeout"), () => false).reason,
		).toBe("timeout");
		expect(
			formatRegistryError(new Error("socket hang up ENOTFOUND"), () => false)
				.reason,
		).toBe("network");
		expect(
			formatRegistryError(new Error("invalid registry metadata"), () => false)
				.reason,
		).toBe("invalid");
		expect(formatRegistryError(new Error("boom"), () => false).reason).toBe(
			"unknown",
		);
	});
	...
	it("isHttpError 无 status 时回退消息判定", () => {
		expect(
			formatRegistryError(httpError(undefined, "boom"), () => true).reason,
		).toBe("unknown");
	});
	...
	it("marketNextReason 挂在非 Error 对象上时消息取字符串化结果", () => {
		const error = { marketNextReason: "timeout" } as {
			marketNextReason: string;
		};
		expect(formatRegistryError(error, () => false)).toEqual({
			reason: "timeout",
			error: String(error),
...
describe("mergeFailureDetail", () => {
	...
	it("全部 not-found 时保持原 detail", () => {
		expect(mergeFailureDetail(detail, ["not-found", "not-found"])).toEqual(
			detail,
		);
	});
	...
	it("存在非 not-found 原因时取首个", () => {
		expect(
			mergeFailureDetail(detail, ["not-found", "timeout", "network"]),
		).toEqual({
			reason: "timeout",
			error: "boom",
		});
