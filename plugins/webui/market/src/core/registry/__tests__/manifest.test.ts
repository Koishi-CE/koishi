...
describe("resolvePluginName", () => {
	it("官方/社区全名原样返回,裸名展开为两个候选", () => {
		expect(resolvePluginName("@koishijs/plugin-foo")).toEqual([
			"@koishijs/plugin-foo",
		]);
		expect(resolvePluginName("koishi-plugin-foo")).toEqual([
			"koishi-plugin-foo",
		]);
		expect(resolvePluginName("@scope/koishi-plugin-foo")).toEqual([
			"@scope/koishi-plugin-foo",
		]);
		expect(resolvePluginName("foo")).toEqual([
			"@koishijs/plugin-foo",
			"koishi-plugin-foo",
		]);
	});
	...
	it("scope 短名展开为 scope 下的插件名", () => {
		expect(resolvePluginName("@scope/foo")).toEqual([
			"@scope/koishi-plugin-foo",
		]);
	});
});
...
describe("Scanner 静态与工厂", () => {
	it("Scanner 静态解包后可直接判定插件与兼容性", () => {
		...
		expect(
			Scanner.isCompatible("^4.0.0", remote("1.0.0", { koishi: "^4.18.0" })),
		).toBe(true);
		expect(
			Scanner.isCompatible("^4.0.0", remote("1.0.0", { koishi: "^3.0.0" })),
		).toBe(false);
	});
