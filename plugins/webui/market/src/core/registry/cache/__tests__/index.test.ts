...
describe("PackageCache.getPackage", () => {
	it("成功:拉取、过滤兼容版本、写两层缓存并广播", async () => {
		...
		const { cache, getRegistry, onFlush, scope } = makeHarness(
			async () => payload,
		);
		...
		expect(Object.keys(cache.fullCache["koishi-plugin-demo"]!)).toEqual([
			"2.0.0",
		]);
		expect(onFlush).toHaveBeenCalled();
...
describe("PackageCache.findVersion", () => {
	it("并行拉取,取第一个有版本结果的包的最新版本", async () => {
		...
		await expect(cache.findVersion(["a", "b"])).resolves.toEqual({
			b: "3.2.1",
		});
	});
