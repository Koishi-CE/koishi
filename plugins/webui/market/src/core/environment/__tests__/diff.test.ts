...
describe("getEnvironmentDiff", () => {
	it("无变化 → unchanged", () => {
		const current = snapshot({
			foo: dep({ request: "^1.0.0", resolved: "1.2.0" }),
		});
	...
	it("added / removed", () => {
		...
		expect(
			getEnvironmentDiff(current, target).find((item) => item.name === "b")!
				.status,
		).toBe("added");
		expect(
			getEnvironmentDiff(target, current).find((item) => item.name === "b")!
				.status,
		).toBe("removed");
	});
	...
	it("本地依赖一致 → unchanged", () => {
		...
		expect(
			getEnvironmentDiff(snapshot({ local }), snapshot({ local }))[0]!.status,
		).toBe("unchanged");
	});
	...
	it("结果按键排序", () => {
		...
		expect(
			getEnvironmentDiff(current, target).map((item) => item.name),
		).toEqual(["a", "b"]);
	});
});
...
describe("getEnvironmentInstallChanges", () => {
	...
	it("unsupported 不产生安装变化", () => {
		const current = snapshot({
			x: dep({ request: "file:../x", source: "file", local: true }),
		});
		const target = snapshot({});
		const diff = getEnvironmentDiff(current, target);
		expect(getEnvironmentInstallChanges(diff, target)).toEqual({});
