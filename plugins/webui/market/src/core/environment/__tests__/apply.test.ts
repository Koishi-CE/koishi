...
import {
	buildEnvironmentDependencies,
	planEnvironmentApply,
} from "../apply.js";
import type {
...
function snapshot(
	deps: Record<string, EnvironmentDependencySnapshot>,
): EnvironmentSnapshot {
	return {
		id: "env-test",
		createdAt: 0,
...
describe("buildEnvironmentDependencies", () => {
	it("去掉 ^/~ 前缀并按本地解析结果回填", () => {
		...
		expect(result["baz"]).toMatchObject({
			request: "file:../baz",
			invalid: true,
		});
	});
	...
	it("非法范围标记 invalid", () => {
		expect(
			buildEnvironmentDependencies({ foo: "not-a-version" }, {})["foo"]!
				.invalid,
		).toBe(true);
	});
});
...
describe("planEnvironmentApply", () => {
	it("返回 diff、unsupported 与安装请求变化", () => {
		...
		expect(plan.diff.map((item) => item.status)).toEqual([
			"upgrade",
			"unsupported",
		]);
	});
