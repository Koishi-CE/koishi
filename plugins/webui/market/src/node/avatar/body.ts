 ...
 * GET 响应后调用。
 */
import type { HTTP } from "@koishi-ce/koishi";
 ...
 */
export async function readAvatarResponse(
	response: HTTP.Response<AvatarBodyStream>,
) {
	...
	if (Number.isFinite(length) && length > AVATAR_MAX_SIZE)
		return cancelAndDiscard(response.data);
