...
const bundleInstallMember = z.object({
	...
	usePreset: z.boolean(),
	conflict: z
		.enum(["same-group", "other-config", "package-mismatch"])
		.optional(),
...
const bundleConfigRemoveRequest = z.object({
	package: z.string(),
	members: z
		.array(z.object({ package: z.string(), plugin: z.string() }))
		.optional(),
...
const contracts = {
	...
	"market/environment-snapshot-apply": z.tuple([
		z.string(),
		installOptions.nullish(),
	]),
