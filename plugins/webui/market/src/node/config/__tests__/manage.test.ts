...
vi.mock("../../installer/index.js", () => ({ SELF_PACKAGE: "@koishi-ce/plugin-marketn" }));
vi.mock("../index.js", () => ({
...
describe("removeLegacyCollapsedGroupsConfig", () => {
    it("删除存在的 collapsedGroups 字段,重复调用返回 false", () => {
        const config = {} as Config;
        const plugins = { "@koishi-ce/plugin-marketn": { collapsedGroups: { core: true } } };
        const { ctx } = context(plugins);
...
describe("updateMarketNextConfig", () => {
    it("按白名单应用 patch 并写盘、刷新 config 视图", async () => {
        ...
        expect(plugins["@koishi-ce/plugin-marketn"]).toMatchObject({ bulkMode: true });
