...
describe("normalizeMarketSilentRules", () => {
	...
	it("新形态规则保留 type/note,enabled 缺省为 true", () => {
		expect(
			normalizeMarketSilentRules([{ type: "preview", value: "" }]),
		).toEqual([{ value: "", enabled: true, type: "preview" }]);
		expect(
			normalizeMarketSilentRules([
				{ type: "custom", value: "v", note: "n", enabled: false },
			]),
	...
	it("value 带 whitespace 时两侧截断", () => {
		expect(
			normalizeMarketSilentRules([{ value: "  2024-01-01  " }])[0]!.value,
		).toBe("2024-01-01");
	});
	...
	it("旧版形态按 value → date → days → query 顺序回退", () => {
		...
		expect(
			normalizeMarketSilentRules([{ date: " 2024-01-01 " }])[0]!.value,
		).toBe("2024-01-01");
		...
		expect(
			normalizeMarketSilentRules([{ query: "category:adapter" }])[0]!.value,
		).toBe("category:adapter");
		// 全部缺省时值为空串
		...
		expect(
			normalizeMarketSilentRules([{ date: "", days: null, query: "q" }])[0]!
				.value,
		).toBe("q");
	});
});
