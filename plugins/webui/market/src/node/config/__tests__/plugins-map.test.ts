...
describe("findPluginConfigKey", () => {
	it("按短名命中普通键", () => {
		expect(findPluginConfigKey({ "chat:anything": {} }, "chat")).toBe(
			"chat:anything",
		);
	...
	it("跳过 $ 元键", () => {
		...
		expect(
			findPluginConfigKey({ $label: "x", chat: {} }, "$label"),
		).toBeUndefined();
		// $ 键不会被当成 chat 的候选,返回真正的业务键
		// biome-ignore lint/style/useNamingConvention: koishi 配置树的 $ 元键形态
		expect(findPluginConfigKey({ $label: "x", "chat:1": {} }, "chat")).toBe(
			"chat:1",
		);
	});

	it("剥掉 ~ 禁用前缀后匹配", () => {
		expect(findPluginConfigKey({ "~chat:ident": {} }, "chat")).toBe(
			"~chat:ident",
		);
	...
	it("scope 名保留在短名中参与比较", () => {
		expect(
			findPluginConfigKey({ "@scope/plugin-x:1": {} }, "@scope/plugin-x"),
		).toBe("@scope/plugin-x:1");
		// 不带 scope 的短名不匹配带 scope 的键
		expect(
			findPluginConfigKey({ "@scope/plugin-x:1": {} }, "x"),
		).toBeUndefined();
	});
	...
	it("group 键本身也可被定位（短名即 group）", () => {
		expect(findPluginConfigKey({ "group:main": {} }, "group")).toBe(
			"group:main",
		);
	});
});
