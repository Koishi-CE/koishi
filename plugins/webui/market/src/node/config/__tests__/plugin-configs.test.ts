...
vi.mock("../../../core/registry/manifest.js", () => ({
	...
	Scanner: {
		isPlugin: (name: string) =>
			/^(?:@[^/]+\/)?koishi-plugin-[0-9a-z-]+$|^@koishijs\/plugin-[0-9a-z-]+$/.test(
				name,
			),
	},
}));
vi.mock("../../../core/utils/async.js", () => ({
	sleep: vi.fn(async () => {}),
}));
vi.mock("../../console/refresh.js", () => ({
	refreshConsole: vi.fn(async () => {}),
}));
vi.mock("../../installer/index.js", () => ({
	SELF_PACKAGE: "@koishi-ce/plugin-marketn",
}));
	...
	return {
		...
		get: (name: string) =>
			name === "console"
				? {
						listeners: {
							"config/request-runtime": { callback: runtimeListener },
						},
					}
				: undefined,
		// 测试内部可触达的桩,便于断言
...
beforeEach(() => {
	...
	vi.mocked(loadManifest).mockImplementation(((...args: unknown[]) => {
		...
		if (!manifest)
			throw Object.assign(new Error("MODULE_NOT_FOUND"), { code: "ENOENT" });
		return manifest;
	}) as never);
});
...
describe("ensurePluginConfig", () => {
	...
	it("合包命名命中即跳过（不读 manifest）", async () => {
		const ctx = makeContext({});
		expect(
			await ensurePluginConfig(ctx as never, "koishi-plugin-pa-demo"),
		).toBe(false);
	...
	it("manifest 带 koishi.bundle 清单视为合包跳过", async () => {
		manifests["koishi-plugin-bundled"] = {
			koishi: {
				bundle: {
					members: [{ package: "koishi-plugin-member", plugin: "member" }],
				},
			},
		};
		const ctx = makeContext({});
		expect(
			await ensurePluginConfig(ctx as never, "koishi-plugin-bundled"),
		).toBe(false);
	});
	...
	it("manifest 关键字含 market:package 视为合包跳过", async () => {
		...
		const ctx = makeContext({});
		expect(
			await ensurePluginConfig(ctx as never, "koishi-plugin-with-keyword"),
		).toBe(false);
	});
	...
	it("manifest 读取失败按非合包继续，走手写禁用条目兜底", async () => {
		...
		expect(await ensurePluginConfig(ctx as never, "koishi-plugin-demo")).toBe(
			true,
		);
	...
	it("配置树已有条目（含 group 嵌套）时不重复创建", async () => {
		...
		expect(await ensurePluginConfig(flat as never, "koishi-plugin-demo")).toBe(
			false,
		);
		const nested = makeContext({ "group:g": { "demo:y": {} } });
		expect(
			await ensurePluginConfig(nested as never, "koishi-plugin-demo"),
		).toBe(false);
	...
	it("write=false 时创建条目但不写盘", async () => {
		const ctx = makeContext({});
		expect(
			await ensurePluginConfig(ctx as never, "koishi-plugin-demo", false),
		).toBe(true);
		expect(ctx.writeConfig).not.toHaveBeenCalled();
	});
});
...
describe("ensurePluginConfigs", () => {
	it("过滤非插件名且全部已有配置时返回 false（不写盘不刷新）", async () => {
		...
		expect(
			await ensurePluginConfigs(ctx as never, [
				"not-a-plugin",
				"koishi-plugin-demo",
			]),
		).toBe(false);
	...
	it("有新建条目时统一写盘并刷新 config/packages 通道", async () => {
		const ctx = makeContext({});
		expect(
			await ensurePluginConfigs(ctx as never, ["koishi-plugin-demo"]),
		).toBe(true);
		expect(ctx.writeConfig).toHaveBeenCalledTimes(1);
		expect(refreshConsole).toHaveBeenCalledWith(ctx as never, [
			"config",
			"packages",
		]);
	});
