...
import {
	...
	runPackageManager,
} from "../../core/install/pipeline/runner.js";
import {
...
import type {
	PackageCache,
	PackageVersions,
} from "../../core/registry/cache/index.js";
...
export const SELF_PACKAGE = "@koishi-ce/plugin-marketn";
 ...
 */
export class Installer extends RegistryStatusMixin(
	UploadsMixin(LogsMixin(Service)),
) {
	...
	override async start() {
		...
		await this.orchestrator
			.recordCurrentEnvironmentSnapshot("startup")
			.catch((error) => {
				this.log.warn(
	...
	async probeDependenciesInBackground(reason = "background") {
		...
		this.log.info(
			`dependency ${reason} probe completed: elapsed=${Date.now() - start}ms`,
		);
	}
