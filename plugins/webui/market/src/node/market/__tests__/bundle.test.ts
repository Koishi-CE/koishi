...
/** 标准请求:单成员勾选,可覆盖成员选项。 */
function requestOf(
	memberOverrides: Partial<BundleInstallMember> = {},
): BundleInstallRequest {
...
function dataStoreStub() {
	...
	} as unknown as MarketDataStore & {
		setBundleRecord: ReturnType<typeof vi.fn>;
	};
}
...
describe("removeBundleConfigs", () => {
	...
	it("分组只剩 $ 元数据且 removeEmptyGroup=false 时不写盘", async () => {
		const plugins = {
			"group:pa-demo-bundle": { ["$label"]: "x", ["$collapsed"]: false },
		};
		const ctx = createMockContext({ plugins });
...
describe("installBundle", () => {
	...
	it("请求缺 version 时抛错", async () => {
		...
		request.version = "";
		await expect(
			installBundle(ctx.asContext(), dataStore, request),
		).rejects.toThrow("bundle package version is required");
	...
	it("registry 元数据未加载时抛错", async () => {
		...
		await expect(
			installBundle(ctx.asContext(), dataStore, requestOf()),
		).rejects.toThrow(`bundle package metadata not loaded: ${PACKAGE}`);
	});

	it("请求版本在 registry 中不存在时抛错", async () => {
		ctx.installer.getRegistry.mockResolvedValue(
			registryWith(undefined, undefined) as never,
		);
		ctx.installer.getRegistry.mockResolvedValue({
		...
		await expect(
			installBundle(ctx.asContext(), dataStore, requestOf()),
		).rejects.toThrow(`bundle package version not found: ${PACKAGE}@1.0.0`);
	});
	...
	it("远端清单缺失 koishi.bundle 时校验失败抛错", async () => {
		...
		await expect(
			installBundle(ctx.asContext(), dataStore, requestOf()),
		).rejects.toThrow(/invalid plugin bundle: .*missing koishi\.bundle/);
	});
	...
	it("成功主链路:安装 override+写配置+记录+刷新", async () => {
		...
		const group = ctx.loader.config.plugins[GROUP_KEY] as Record<
			string,
			unknown
		>;
	...
	it("beforeDeps 已有成员依赖时 installedByBundle 为 false", async () => {
		...
		expect(result.record?.members[0]).toMatchObject({
