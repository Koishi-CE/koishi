...
import type {
	MarketIndexSource,
	MarketSourceDeps,
} from "../../../core/market/source/index.js";
...
describe("MarketProviderConfig schema", () => {
	it("默认值:endpoint/timeout/autoRoute/logLevel(断言 Schema 结构描述字段)", () => {
		...
		const dict = MarketProviderConfig.dict as Record<
			string,
...
describe("MarketProvider", () => {
	...
	it("ready 事件触发 warmDiskCache(startup)", () => {
		const provider = new MarketProvider(ctx.asContext());
		const warm = vi
			.spyOn(sourceOf(provider), "warmDiskCache")
	...
	describe("createDeps 接线", () => {
		...
		it("timeout 未配置时 extend 参数不含 timeout 键", async () => {
			const provider = new MarketProvider(ctx.asContext());
			ctx.http.extend.mockImplementation(() =>
			...
			expect(
				Object.hasOwn(
					vi.mocked(ctx.http.extend).mock.calls[0]?.[0] as object,
					"timeout",
				),
			).toBe(false);
		});
		...
		it("scannerRequest/notifyRefresh/onRegistryVersions 直接透传", async () => {
			...
			await deps.scannerRequest("https://registry.example/-/all", {
				timeout: 8,
			});
			expect(ctx.http.get).toHaveBeenCalledWith(
				"https://registry.example/-/all",
				{
					timeout: 8,
				},
			);
			...
			expect(ctx.installer.setPackage).toHaveBeenCalledWith(
				"koishi-plugin-foo",
				versions,
			);
		});
		...
		it("缓存文件路径按 baseDir 派生", () => {
			...
			expect(deps.cacheFile).toBe(
				resolve("/mock-base", "cache", "market-next-index.json"),
			);
			expect(deps.cacheDir).toBe(
				resolve("/mock-base", "cache", "market-next-index"),
			);
		});
		...
		it("log 门控:silent 全静默,error 时 warn 静默、info 级时 debug 静默", () => {
			const silent = new MarketProvider(ctx.asContext(), {
				logLevel: "silent",
			});
	...
	describe("get() 字段映射", () => {
		...
		it("logLevel=debug 时暴露 debug 信息,否则不暴露", async () => {
			...
			expect((await provider.get()).debug).toMatchObject({
				timings: { total: 42 },
			});
