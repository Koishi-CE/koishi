...
vi.mock("../../../core/utils/atomic-write.js", () => ({
	...
	it("正常读取并归一化文件内容", async () => {
		await writeStoreFile(
			JSON.stringify({
				...
				bundleRecords: {
					"koishi-plugin-pa-demo": { package: "koishi-plugin-pa-demo" },
				},
...
describe("MarketDataStore", () => {
	it("构造走真实 DataService 基类:immediate 服务名为 console.services.marketData", () => {
		...
		expect((store as unknown as { name: string }).name).toBe(
			"console.services.marketData",
		);
	});
	...
	it("patch 合并四类 dict 并广播+防抖写盘", async () => {
		...
		expect(writeMock.mock.calls[0]?.[0]).toBe(
			dataFilePath(dir, "market-next.json"),
		);
	...
	describe("migrateFromConfig", () => {
		...
		it("文件已有 collapsedGroups 键时不迁移配置侧值,版本达标后不再执行清理", async () => {
			...
			await store.migrateFromConfig({
				collapsedGroups: { installed: false, other: true },
			});
