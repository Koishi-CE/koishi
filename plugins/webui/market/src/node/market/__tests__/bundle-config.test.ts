...
function member(
	overrides: Partial<BundleInstallMember> = {},
): BundleInstallMember {
	return {
...
function manifestOf(
	...
	return {
		label,
		members: members.map(
			({ package: pkg, plugin, version, config, required }) => ({
				package: pkg,
				plugin,
				version,
				config,
...
describe("createBundleConfigWriter", () => {
	...
	it("组外同短名藏在嵌套 group 里且勾 move:递归找到并搬移", async () => {
		...
		expect(writer.moved).toEqual(["koishi-plugin-foo"]);
		expect(
			(ctx.loader.config.plugins["group:other"] as Record<string, unknown>)[
				"foo:deep"
			],
		).toBeUndefined();
		expect(
	...
	describe("属性可枚举性边界(组探测用 for...in,键占用判定用 in)", () => {
		...
		function groupWithHiddenKeys() {
			...
			Object.defineProperty(group, "foo", {
				value: { g1: true },
				enumerable: false,
			});
		...
		it("搬移的原键与合成键都被不可枚举键占用时记跳过", async () => {
			...
			const ctx = createMockContext({
				plugins: { foo: { top: true }, [GROUP_KEY]: group },
			});
			const writer = createBundleConfigWriter(
				ctx.asContext(),
				requestOf(members),
