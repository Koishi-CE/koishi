 ...
 */
import { type Context, Time } from "@koishi-ce/koishi";
...
export function setupIdleProbe(ctx: Context, config: Config) {
	...
	const getBootDelay = () =>
		Math.max(0, config.idleProbeBootDelay ?? Time.minute);
	const getInterval = () =>
		Math.max(0, config.idleProbeInterval ?? Time.hour * 6);
	...
	const schedule = (delay = getDelay()) => {
		...
		logger.debug(
			`idle background probe scheduled: delay=${Math.max(0, delay)}ms`,
		);
	};
	...
	const getProbeWait = (): number | undefined => {
		...
		const intervalWait = lastProbe
			? getInterval() - (Date.now() - lastProbe)
			: 0;
	...
	const runProbeTasks = async (): Promise<{
		succeeded: boolean;
		reason?: unknown;
	}> => {
	...
	const runProbe = async () => {
		...
		try {
			...
			if (!getClientCount())
				schedule(
					lastProbe ? getInterval() : Math.min(Time.minute * 5, getInterval()),
				);
		}
	};
	...
	ctx.on("console/connection", () => {
		if (getClientCount()) {
			clearIdleTimer();
			logger.debug(
				`idle background probe cancelled: clients=${getClientCount()}`,
			);
		} else {
			schedule();
		}
