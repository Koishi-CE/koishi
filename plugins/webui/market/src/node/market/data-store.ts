...
export async function readMarketDataStore(
	ctx: Context,
...
export class MarketDataStore extends DataService<MarketDataStorePayload> {
	...
	async migrateFromConfig(config: MarketMigrationConfig) {
		...
		if (migratedVersion !== undefined)
			this.collapsedGroupsVersion = migratedVersion;
	...
	private async load() {
		try {
			...
			this.collapsedGroupsVersion = Number.isInteger(
				value?.collapsedGroupsVersion,
			)
