...
export function getBundleGroup(
	ctx: Context,
	packageName: string,
...
function findPluginConfig(
	...
	for (const key in (plugins as PluginConfigMap) ?? {}) {
		...
		if (name === shortname)
			return { key, parent: plugins as PluginConfigMap, value };
		if (name === "group") {
...
async function writeBundleConfig(
	...
	for (const member of selected) {
		groupChanged =
			configureBundleMember(ctx, request, manifest, member, writer) ||
			groupChanged;
	}
...
function configureBundleMember(
	...
	if (hasPluginConfigInGroup(writer.group.plugins, shortname))
		return groupChanged;
