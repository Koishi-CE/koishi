...
- 3a22c30: 自研依赖管理页(/dependencies):分组卡片墙按待应用/工作区/非法声明/拉取失败/可更新/已安装六态分类展示,选版本与移除依赖经前端暂存后批量应用(失败自动还原 package.json),支持忽略更新(限时/限版本/永久)与预发布屏蔽;registry 元数据并发拉取上限 4,404 会话内负缓存,网络错误自动重试。

### Patch Changes

- 69233d4: 市场卡片认证徽章改为右对齐独占一行、参与文档流占位：描述过长的插件不再出现第三行文字与右下角徽章重叠，徽章卡片描述自动少渲染一行，无徽章卡片外观与页脚位置不变。
- @koishi-ce/koishi@1.0.18
...
- 6a69e74: 补全插件词典六语言真翻译：de-DE / fr-FR / ja-JP / ru-RU 此前为上游 Crowdin 未翻的中文占位物，现补为真翻译；en-US 清除七处残留中文；zh-TW 修正简体残留并将转换器直转词改为台湾惯用词（预设 / 專案 / 資料等）。
...
- c9f2307: 市场页面前端视觉重设计（借鉴 marketn 设计语言）：类目色渐变图标块、胶囊徽章（悬浮于卡片右下头像列上方，避免与长插件名挤压折行）、心跳新鲜度指示、搜索框聚焦光环与主色调查询词、筛选栏 active 胶囊态、慢加载提示与错误态重试按钮、品牌化滚动条。核心视图组件已本地化 fork（逻辑层仍复用 @koishijs/market 包），并顺修上游筛选栏「重复点击已选分类无法取消」的问题。

## 1.0.14

### Patch Changes

- a5c0691: 安装完成后校验新装包的依赖链接完整性,缺失时自动补装一次。Bun isolated 布局下增量安装存在缺陷:升级插件时包目录 hash 变化而其依赖解析结果不变,bun 会跳过新包目录内的依赖链接建立,表现为插件入口可加载、内部依赖 require 报 MODULE_NOT_FOUND 且普通 bun install 报 no changes 不修复。校验全程纯 fs(existsSync / readFileSync / realpathSync),不触碰解析 API;补装利用 bun 按磁盘实际状态重建链接的行为,秒级自愈。

## 1.0.13

### Patch Changes

- 16a65a6: 修复市场索引抓取遇网络层瞬态错误（证书验证失败、连接重置等，常见于代理节点切换抖动）时中断采集并清空市场页的问题：此类错误现与超时一样按指数退避重试。

## 1.0.12

### Patch Changes

- 92418f5: market 索引请求的退避重试覆盖超时与镜像瞬态 404
