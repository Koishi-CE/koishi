...
async function call<K extends keyof Console["listeners"]>(
	...
	if (!listener)
		throw new Error(`listener ${event} not found`);
...
describe("sandbox 插件", () => {
	it("加载数据服务与浏览器侧 RPC 监听器", () => {
		expect(
			app.get("console.services.sandbox"),
		).toBeDefined();
		for (const event of [
	...
	it("send-message 回显消息并派发沙盒会话", async () => {
		...
		// 沙盒 bot 已按平台登记且隐藏
		const bot = app.bots["sandbox:web:koishi"] as
			| SandboxBot
			| undefined;
	...
	it("clear 指令通知浏览器清空消息", async () => {
		...
		expect(
			sent.some(
				(message) => message.type === "sandbox/clear",
			),
		).toBe(true);
	});
	...
	it("set-user 触发成员事件并按数据更新 / 删除用户", async () => {
		...
		const removed = await app.database.getUser(
			"sandbox:web",
			"123",
		);
		...
		const none = await app.database.getUser(
			"sandbox:web",
			"404",
		);
	...
	it("response 监听器把浏览器应答转发为应用事件", async () => {
		...
		await call("sandbox/response", client, "nonce-1", {
			ok: true,
		});
		dispose();
	...
	it("SandboxBot.request 以 nonce 关联应答", async () => {
		...
		const task = bot.request<{ size: number }>(
			"getChannelList",
			{
				guildId: "g",
			},
		...
		const nonce = (request as { body: { nonce: string } })
			.body.nonce;
	...
	it("SandboxBot 的主动 API 封装 request", async () => {
		...
		const request = sent.find(
			(message) => message.type === "sandbox/request",
		);
	...
	it("SandboxBot 其余主动 API 同样经 request 转发", async () => {
		...
		for (const message of sent) {
			...
			app.emit(
				"sandbox/response",
				(message.body as { nonce: string }).nonce,
				{
					data: true,
				} as never,
			);
	...
	it("SandboxBot.request 超时后拒绝", async () => {
		...
		await expect(bot.request("never")).rejects.toThrow(
			"timeout",
		);
	}, 8000);
	...
	it("SandboxMessenger 按分段 flush 并推回浏览器", async () => {
		...
		const encoder = new SandboxMessenger(
			bot,
			"#main",
			undefined,
			{
		...
		await encoder.send(
			"第一段<message>第二段</message>第三段",
		);
		const messages = sent.filter(
			(m) => m.type === "sandbox/message",
		);
		...
		expect(messages[1]?.body).toMatchObject({
			content: "第二段",
		});
		expect(messages[2]?.body).toMatchObject({
			content: "第三段",
		});
	});
	...
	it("SandboxMessenger 把 file: 媒体重写到沙盒文件服务", async () => {
		...
		const content = (
			message as { body: { content: string } }
		).body.content;
		...
		const direct = sent.find(
			(m) => m.type === "sandbox/message",
		);
		expect(
			(direct as { body: { content: string } }).body
				.content,
		).toContain("https://example.com/pic.png");
	});

	it("数据服务按平台统计绑定用户数", async () => {
		await app.database.createUser("sandbox:stats", "u1", {
			authority: 1,
		});
		const svc = app.get("console.services.sandbox");
		const data = (await svc?.get()) as Record<
			string,
			number
		>;
	...
	it("新连接接入时清理已失联平台的沙盒 bot", async () => {
		...
		expect(
			app.bots["sandbox:cleanup:koishi"],
		).toBeDefined();
