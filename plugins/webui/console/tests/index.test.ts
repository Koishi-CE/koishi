...
import NodeConsole, { rewriteSharedImports } from "../src/node/index.ts";
...
describe("rewriteSharedImports", () => {
	...
	test("带空格的静态导入被改写", () => {
		expect(rewriteSharedImports('import d from "vue-router";\n')).toBe(
			'import d from "../vue-router.js";\n',
		);
	});

	test("无绑定名的副作用导入被改写（曾致 logger 插件整模块加载失败）", () => {
		expect(rewriteSharedImports('import"vue-router";var y=1;')).toBe(
			'import"../vue-router.js";var y=1;',
		);
		expect(rewriteSharedImports('import "vue-router";\nvar y=1;')).toBe(
			'import "../vue-router.js";\nvar y=1;',
		);
	});

	test("再导出形态被改写", () => {
		expect(rewriteSharedImports('export{useRouter}from"vue-router";')).toBe(
			'export{useRouter}from"../vue-router.js";',
		);
	...
	test("动态导入被改写", () => {
		expect(rewriteSharedImports('const m=import("vue");')).toBe(
			'const m=import("../vue.js");',
		);
	});

	test("上游包名映射到同一份共享 client", () => {
		expect(rewriteSharedImports('import{send}from"@koishijs/client";')).toBe(
...
const resolveEntry = (
	NodeConsole.prototype as unknown as {
		resolveEntry: (this: FakeHost, files: Entry.Files, key: string) => string[];
	}
).resolveEntry;
...
describe("resolveEntry", () => {
	...
	test("产物目录含 style.css 时按约定名下发", () => {
		...
		expect(resolveEntry.call(host(), { dev: "", prod: dir }, "key1")).toEqual([
	...
	test("旧版插件包只有 index.css 时兜底下发（存量 1.0.0 包样式修复）", () => {
		...
		expect(resolveEntry.call(host(), { dev: "", prod: dir }, "key2")).toEqual([
	...
	test("两个样式并存时 style.css 优先", () => {
		...
		writeFileSync(join(dir, "index.css"), "");
		writeFileSync(join(dir, "style.css"), "");
		expect(resolveEntry.call(host(), { dev: "", prod: dir }, "key3")).toEqual([
	...
	test("无样式产物只下发入口脚本", () => {
		...
		mkdirSync(dir, { recursive: true });
		expect(resolveEntry.call(host(), { dev: "", prod: dir }, "key4")).toEqual([
			"/@plugin-key4/index.js",
		]);
	});

	test("数组形态的具名文件直接映射 URL，不追加探测", () => {
		expect(
			resolveEntry.call(host(), ["/pkg/dist/index.js", "/pkg/dist/a.css"], "k"),
	...
	test("devMode 走 /vite/@fs/ 前缀（目录形态同样追加入口名）", () => {
		...
		expect(
			resolveEntry.call(host(true), { dev: "", prod: dir }, "key5"),
		).toEqual([`/vite/@fs/${dir}/index.js`, `/vite/@fs/${dir}/style.css`]);
	});
