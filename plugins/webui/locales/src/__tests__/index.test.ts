...
import { App, type Plugin, type Universal } from "@koishi-ce/koishi";
...
/** 建立带初始文件的临时根目录 */
async function makeRoot(name: string, files: Record<string, string>) {
	const dir = join(tmpdir(), `koishi-locales-${name}-${Date.now()}`);
...
const mountConsole = (target: App) =>
	target.plugin(TestConsole as unknown as Plugin.Constructor<App>);
...
const localesFork = app.plugin(locales, { root: [root1, root2] });
...
/** i18n 存储视图（点路径扁平字典） */
function store() {
	return app.i18n._data as Record<string, Record<string, string>>;
}

describe("@koishi-ce/plugin-locales", () => {
	it("扫描根目录的 yml 注册 i18n（非 yml 文件跳过）", () => {
		expect(store()["$zh-CN"]?.["greeting.hello"]).toBe("你好");
		expect(store()["$ja-JP"]?.["greeting.hello"]).toBe("こんにちは");
	});
	...
	it("entry 数据携带全部 i18n 存储，文件声明默认走 dev/prod 形态", async () => {
		...
		const storeData = entryData?.data as Record<string, unknown>;
		expect(storeData["$zh-CN"]).toBeTruthy();

		const files = entry?.files as Entry.EntryOptions;
		// Windows 分隔符归一后断言（本用例的 prod 声明恒为字符串形态）
		expect(files.dev.replace(/\\/g, "/").endsWith("client/index.ts")).toBe(
			true,
		);
		expect((files.prod as string).replace(/\\/g, "/").endsWith("dist")).toBe(
			true,
		);
	});
	...
	it("l10n 监听器更新 i18n 并写回第一个根目录", async () => {
		...
		const content = await readFile(join(root1, "zh-CN.yml"), "utf8");
	...
	it("KOISHI_BASE 环境下 entry 指向在线加载地址", async () => {
		...
		await tick();
		const entry = Object.values((localApp.console as TestConsole).entries)[0];
	...
	it("KOISHI_ENV=browser 环境下 entry 指向 client 源码", async () => {
		...
		expect(entry?.files).toEqual([
			new URL("../../client/index.ts", import.meta.url).href,
		]);
		await localApp.stop();
		if (env === undefined) {
	...
	it("空根目录列表时跳过扫描且 l10n 监听器不落盘", async () => {
		...
		expect(
			(localApp.i18n._data as Record<string, unknown>)["$zz-ZZ"],
		).toBeUndefined();
		await localApp.stop();
	});
});
