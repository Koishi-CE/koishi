...
const service = () =>
	app.get("console.services.insight") as Insight;
...
beforeAll(async () => {
	...
	app.plugin(
		injectedPlugin as unknown as Plugin.Object<App>,
	);
	app.plugin((_ctx) => {});
});
...
describe("insight 依赖图服务", () => {
	...
	it("生成包含根应用与各插件节点的图数据", async () => {
		...
		expect(
			nodes.filter((node) => node.name === "ReusableA"),
		).toHaveLength(2);
		// normal 插件只有一个主状态节点
		expect(
			nodes.filter((node) => node.name === "NormalB"),
		).toHaveLength(1);

		// 存在实线（fork 调用）与虚线（服务注入）两类边
		expect(
			edges.some((edge) => edge.type === "solid"),
		).toBe(true);
		expect(
			edges.some((edge) => edge.type === "dashed"),
		).toBe(true);

		// dashed 边的源是 console 服务所在 scope
		const dashed = edges.filter(
			(edge) => edge.type === "dashed",
		);
		const dashedTargets = dashed.map((edge) => edge.target);
		const injectedNode = nodes.find(
			(node) =>
				node.name === "InjectedC" ||
				node.name === "injected-c",
		);
