...
import explorer, { type Entry as TreeEntry } from "./index.ts";
...
async function createScenario() {
	const root = mkdtempSync(join(tmpdir(), "explorer-test-"));
	await writeFile(join(root, "b.txt"), "hello world", "utf8");
	...
	await writeFile(join(root, "sub", "c.txt"), "nested", "utf8");
	await mkdir(join(root, "node_modules"));
	await writeFile(join(root, "node_modules", "x.txt"), "ignored");
	await writeFile(join(root, ".hidden"), "ignored");
	// win32 无特权环境下用 junction 目录链接充当符号链接
	await symlink(join(root, "sub"), join(root, "link"), "junction");
	return root;
}
...
async function call<K extends keyof Console["listeners"]>(
	...
	if (!listener) throw new Error(`listener ${String(event)} not found`);
	return listener.callback.call({} as never, ...(args as never[]));
}
...
describe("explorer 文件管理服务", () => {
	it("文件树遍历：目录优先、按名排序、忽略隐藏与 node_modules", async () => {
		...
		expect(names).toEqual(["sub", "a.png", "b.txt", "link"]);
		const sub = entries[0];
		expect(sub?.type).toBe("directory");
		expect(sub?.children?.map((child) => child.name)).toEqual(["c.txt"]);
	...
	it("read 返回文本文件的 base64 与编码探测", async () => {
		...
		expect(Buffer.from(file.base64, "base64").toString("utf8")).toBe(
			"hello world",
		);
	...
	it("write 支持文本与 base64 二进制写入", async () => {
		...
		expect(await readFile(join(root, "sub", "new.txt"), "utf8")).toBe("新内容");
		...
		expect(await readFile(join(root, "sub", "new.bin"))).toEqual(buffer);
	});
	...
	it("explorer/refresh 与 get 的缓存复用语义", async () => {
		...
		expect(fresh.map((entry) => entry.name)).toContain("z.txt");
	});

	it("read 不存在的文件时抛错", async () => {
		expect(call("explorer/read", "not-exist.txt")).rejects.toThrow();
	});
});
