...
app.plugin(
	notifier as unknown as Plugin.Constructor<App>,
	{},
);
...
describe("notifier 服务", () => {
	it("加载后注册控制台入口与按钮监听", () => {
		...
		expect(
			app.console.listeners["notifier/button"],
		).toBeDefined();
	});
	...
	it("button 的 onClick 被摘出为 key，点击回调经监听器触发", async () => {
		...
		expect(
			typeof service().actions[keys[0] as string],
		).toBe("function");
		expect(n.toJSON().content).toContain(
			`on-click="${keys[0]}"`,
		);
