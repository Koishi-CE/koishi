...
import type { Driver, Eval, Model, RegExpLike } from "minato";
...
export class SQLiteBuilder extends Builder {
	...
	constructor(driver: Driver, tables?: Dict<Model>) {
		...
		this.evalOperators.$regex = ([key, value, flags]) => {
			...
			const selfFlags = reg instanceof RegExp ? reg.flags : undefined;
			if (flags?.includes("i") || selfFlags?.includes("i")) {
		...
		this.evalOperators.$in = ([key, value]) =>
			this.asEncoded(this.createMemberEval(key, value, ""), false);
		this.evalOperators.$nin = ([key, value]) =>
			this.asEncoded(this.createMemberEval(key, value, " NOT"), false);
	...
	protected createMemberEval(rawKey: unknown, value: unknown, notStr = "") {
		...
		if (Array.isArray(value)) {
			...
			(Type.fromTerm(value as Eval.Term<unknown>)?.type as string) === "list"
		) {
	...
	protected override createElementQuery(key: string, value: unknown) {
	...
	protected override listContains(list: string, value: string) {
	...
	protected override jsonContains(obj: string, value: string) {
	...
	protected override encode(
		...
		return encoded
			...
			: encoded === this.isEncoded() && !pure
				? value
				: this.asEncoded(
						this.transform(`(${value} ->> '$')`, type, "decode"),
	...
	protected override transformJsonField(obj: string, path: string) {
