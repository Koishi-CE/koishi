...
export async function remove(
	...
	const result = driver._run(
		...
		[],
		() =>
			driver._get(`SELECT changes() AS count`) as {
				count: number;
			},
	) as { count: number };
...
export async function evaluate(
	...
	const inner = builder.get(
		sel.table as Selection,
		true,
		true,
	);
	const output = builder.parseEval(expr, false);
	const { value } = driver._get(
		`SELECT ${output} AS value FROM ${inner}`,
		[],
		{
			useBigInt: true,
		},
...
function updateRow(
	...
	const query = Object.fromEntries(
		indexFields.map((key) => [key, row[key]]),
	);
	const filter = builder.parseQuery(query);
	const args = updateFields.map(
		(key) => row[key] ?? null,
	) as Parameters<StatementSync["run"]>;
	driver._run(
...
export async function set(
	...
	if (
		...
		const sel2 = driver.database.select(
			table as never,
			query,
		);
		...
		const project = mapValues(
			...
			(value, key) => () =>
				isEvalExpr(value)
					? value
					: Eval.literal(value, model.getType(key)),
		);
					...
					primaryFields.map((x) => [
						...
						() =>
							Eval(
								"",
								[sel.ref, x],
								sel2.model.getType(x) as Parameters<
									typeof Eval
								>[2],
							),
					]),
		...
		const upsert = rawUpsert.map((row) => ({
			...
			...Object.fromEntries(
				primaryFields.map((x) => [x, getCell(row, x)]),
			),
		}));
		return driver.database.upsert(table as never, upsert);
	} else {
		const data = await driver.database.get(
			table as never,
			query,
		);
