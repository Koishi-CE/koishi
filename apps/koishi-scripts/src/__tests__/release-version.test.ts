...
const workspaceRoot = mkdtempSync(join(tmpdir(), "koishi-version-run-"));
...
const calls: Array<{ dir: string; cmd: string; args: readonly string[] }> = [];
...
const { default: runVersion } = await import("../release/version.ts");
...
function seedProject(name: string, changesetFiles: string[] = []): string {
...
describe("runVersion", () => {
	...
	it("external/ 下无任何项目（无 package.json）时返回 0", () => {
		resetWorkspace();
		mkdirSync(join(workspaceRoot, "external", "only-a-dir"), {
			recursive: true,
		});
	...
	it("无项目 / 无 pending changeset 的项目均跳过", () => {
		...
		expect(runVersion()).toBe(0);
		expect(calls).toHaveLength(0);
		expect(logs.join("\n")).toContain("无 pending changeset，跳过");
	});

	it("有 pending 条目的项目执行 changeset version（项目本地 bin 优先）", () => {
		resetWorkspace();
		const dir = seedProject("with-pending", ["fix-something.md"]);
		const localBin = join(dir, "node_modules", ".bin", `changeset${binExt}`);
		...
		expect(logs.join("\n")).toContain("1/1 个项目消费了 changeset");
	});
	...
	it("项目本地无 bin 时回退工作区根 node_modules/.bin", () => {
		...
		mkdirSync(join(dir, "node_modules", ".bin", `changeset${binExt}`), {
			recursive: true,
		});
		const rootBin = join(
			workspaceRoot,
