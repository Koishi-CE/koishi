...
const workspaceRoot = mkdtempSync(join(tmpdir(), "koishi-build-run-"));
...
const { default: runBuild } = await import("../release/build.ts");
...
describe("runBuild", () => {
	...
	it("external/ 下无可构建项目时返回 0", () => {
		...
		seedProject("no-build-script", { name: "x", scripts: { test: "x" } });
		expect(runBuild()).toBe(0);
		expect(calls).toHaveLength(0);
		expect(logs.join("\n")).toContain("无 build 脚本，跳过");
	});

	it("按目录序串行构建 yarn 与 pnpm 项目", () => {
		resetWorkspace();
		seedProject("aaa-yarn", { name: "x", scripts: { build: "tsc" } });
		seedProject("zzz-pnpm", { name: "y", scripts: { build: "tsc" } }, [
			"pnpm-lock.yaml",
		]);
		expect(runBuild()).toBe(0);
	...
	it("任一项目失败即中断并透传退出码", () => {
		resetWorkspace();
		seedProject("aaa-fail", { name: "x", scripts: { build: "tsc" } });
		seedProject("zzz-never", { name: "y", scripts: { build: "tsc" } });
