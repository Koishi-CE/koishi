...
const workspaceRoot = mkdtempSync(join(tmpdir(), "koishi-setup-run-"));
...
function seedSibling(name: string, owner: string): void {
	...
	writeFileSync(
		...
		JSON.stringify({
			...
			repository: { url: `git+https://github.com/${owner}/x.git` },
		}),
	);
}
...
function readManifest(...rel: string[]): Record<string, unknown> {
	return JSON.parse(
		readFileSync(join(workspaceRoot, "external", ...rel), "utf8"),
...
describe("runSetup 主流程", () => {
	it("非交互单包形态：全参数生成 + 宿主版本注入", async () => {
		...
		expect(manifest["peerDependencies"]).toEqual({ koishi: "^9.9.9" });
		expect(manifest["devDependencies"]).not.toHaveProperty("@koishijs/client");
		...
		// changeset 配置带主分支字段
		const changeset = readManifest("demo", ".changeset", "config.json");
	...
	it("console 形态：client/ 目录 + owner 众数探测 + 兜底版本", async () => {
		...
		const code = await runSetup(["--console", "--name=web"]);
		expect(code).toBe(0);
	...
	it("monorepo 形态：仓库根 + packages/ 子包两级结构", async () => {
		...
		const rootManifest = readManifest("mono", "package.json");
		...
		const member = readManifest("mono", "packages", "mono", "package.json");
		...
		const scripts = member["scripts"] as Record<string, unknown>;
		...
		expect(logs.join("\n")).toMatch(/已初始化 git 仓库（分支 \S+）/);
		expect(existsSync(join(root, "packages", "mono", ".git"))).toBe(false);
	});
	...
	it("非法包名报错", async () => {
		await expect(runSetup(["--name=bad name!"])).rejects.toThrow(
			"非法的包名：bad name!",
		);
	});
...
describe("目录清理核对", () => {
	it("临时工作区已删除", () => {
		...
		expect(readdirSync(workspaceRoot).length).toBeGreaterThanOrEqual(0);
	});
});
