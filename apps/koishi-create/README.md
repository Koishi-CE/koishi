...
- **开发工具链**：devDependencies 预置 `@koishi-ce/client`、`@koishi-ce/console-builder`（控制台 devMode 的 Vite 开发服务器）、`@koishi-ce/plugin-hmr` 与 `@koishi-ce/scripts`（`koishi-scripts setup` 可在 external/ 下继续创建插件项目）。
...
- **Toolchain** — devDependencies include `@koishi-ce/client`, `@koishi-ce/console-builder` (the Vite dev server behind the console devMode), `@koishi-ce/plugin-hmr` and `@koishi-ce/scripts` (`koishi-scripts setup` scaffolds further plugin projects under external/).
