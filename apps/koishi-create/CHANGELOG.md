...
- ddc4cfe: 模板预置 `overrides` 兜底层：41 个上游名强制重定向到 CE 对应包。
...
- 9ab750c: 模板新增 `@koishijs/client` / `@koishijs/components` 两行上游名钉名（指向新 shim 包 `@koishi-ce/client-shim` / `@koishi-ce/components-shim`）。
