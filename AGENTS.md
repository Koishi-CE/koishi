...
10. **create-koishi-ce 的默认模板是内置的纯 `@koishi-ce` 模板**（`apps/koishi-create/src/template.ts`），不再下载上游官方 `@koishijs/boilerplate`——那会生成全官方生态的项目（正是被修掉的 bug）。`--template <包名>` 保留为远程模板逃生舱。模板要点：Bun 运行时、上游名 alias 钉 shim（koishi 裸名见第 9 条，console/core 见第 11 条）、依赖统一 `^1.0.0` 区间、不预装 adapter / database（本仓无再分发，官方版可后续从市场装，peer 已被 alias 钉住）。
11. **上游 peer 名的占位 shim 族（勿删、勿 bump 版本）**：`@koishijs/plugin-console`（`packages/node/upstream-plugin-console`，workspace 占位、private）、`@koishijs/core`（`packages/node/upstream-core`，同理）与 `@koishijs/loader`（`packages/node/upstream-loader`，同理）让本仓 webui 插件与 loader 声明的上游名 peer 有唯一归属——没有它们时 Bun 的 peer 自动安装会把 npm 官方 console / core / loader 全家桶拉进 node_modules，与 `@koishi-ce/*` 形成双实例。下游对应物是 `@koishi-ce/console-shim`（`packages/node/console-shim`，冻结 5.30.x 线满足 `^5.30.11` peer）、`@koishi-ce/core-shim`（`packages/node/core-shim`，冻结 4.18.11——loader 的 peer 是精确版本，模板 alias 声明**不带 `^`**）与 `@koishi-ce/loader-shim`（`packages/node/loader-shim`，冻结 4.6.x 线满足 `^4.6.11` peer），create-koishi-ce 模板以 npm alias 引用（机制同 koishi 裸名 shim，market 护栏的 `npm:@koishi-ce` 前缀天然覆盖）。根 package.json dependencies 必须保留 `"@koishijs/plugin-console": "workspace:*"`、`"@koishijs/core": "workspace:*"` 与 `"@koishijs/loader": "workspace:*"`（丢了 shim 形同虚设，peer 立即回归 npm 官方包）；六包均在 changesets ignore 与根 tsdown exclude；shim 的 package.json 带 `"koishi-ce": { "upstreamShim": true }` 私有标记，`LocalScanner` 据此跳过（防止与 @koishi-ce 同类插件短名冲突）。发布顺序：console-shim / core-shim / loader-shim 必须先于引用它们的 create-koishi-ce 发布。
