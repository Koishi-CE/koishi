...
7. **market 插件为上游原版再分发**：`plugins/webui/market/`（`@koishi-ce/plugin-market`）对齐自上游 webui `plugins/market`（原版 v2.11.11），社区版 `plugin-marketn` 已被其取代并移除。client 逻辑层自 `@koishijs/market` 4.2.10 vendor 进 `client/vendor/market/`（AGPL 同许可，对 npm 包的依赖已解除）；**vendor 图标子树已按用户授权（2026-09-25）豁免上游纪律收敛进集中 .svg 资产**——`client/vendor/market/icons/` 目录已删除，注册表为 `client/vendor/market/icons.ts`（`<market-icon>` 渲染层与 name 契约不变，6 个与主库同语义的图标直接复用集中资产）；视图四组件（`client/market/`）为本地化 fork，上游同步时须手动 diff 勿整覆盖；包根 `locales/` 词典为本地真翻译（上游原版系 Crowdin 机器人写入的未翻中文占位物），同步时同样手动 diff 勿整覆盖。宿主构建保留 `@koishijs/components` alias 作下游防御：第三方插件以 npm 名引用组件库时重定向到本仓 workspace 版，避免双实例。
...
9. **create-koishi-ce 的默认模板是内置的纯 `@koishi-ce` 模板**（`apps/koishi-create/src/template/` 目录），不下载上游官方 `@koishijs/boilerplate`；`--template <包名>` 保留为远程模板逃生舱。模板与 sandbox 生成器预置六行钉名 + 41 名 overrides 兜底（语义见 `packages/shim/README.md`「三层防线」；两处清单刻意克隆、对账测试防漂移，新增再分发包须两处同步）。
