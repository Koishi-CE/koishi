...
- `peerDependencies` 指向上游包名（`koishi`、`@koishijs/*`）是刻意设计，不要改成 `@koishi-ce/*`。根包名 `koishi` 与上游包名撞名，故 `bumpVersionsWithWorkspaceProtocolOnly: true`（内部关联只认 workspace: 协议，上游名 peer 不参与版本联动）且 `ignore` 掉根包，防止误连带与误报警告。
