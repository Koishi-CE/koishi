...
- 参考手册（按需查阅）：`docs/DEVELOPMENT.md`（开发：环境 / 门禁 / 编码约定 / 测试 / 已知坑细节）、`docs/ARCHITECTURE.md`（结构：包清单 / 构建体系 / 依赖纪律）、`docs/RELEASE.md`（发布）、`docs/UPSTREAM.md`（上游映射与 port 流程）。

## 工作流

1. 收到任务后，先以实际代码为准了解现状（目录与上游的对应关系查 `docs/UPSTREAM.md`），再动手。
...
4. port 上游改动：按 `docs/UPSTREAM.md` 映射表手动 diff 移植，相对导入补 `.ts` 扩展名，完成后 `bun run build` + `bun test` 验证。
