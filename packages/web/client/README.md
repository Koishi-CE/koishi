...
本包没有独立的运行时发布形态，而是作为源码被前端工程消费：`src/` 由 console 打包器并成插件运行时共享块 `client.js`，宿主 SPA（[`@koishi-ce/console-app`](https://github.com/Koishi-CE/koishi/tree/main/packages/web/app)）与各插件均以 external 依赖共享同一份实例，避免多份 Vue 实例。
...
The package is consumed as source rather than shipped as a standalone runtime: `src/` is bundled by the console builder into the shared `client.js` chunk, so that the host SPA ([`@koishi-ce/console-app`](https://github.com/Koishi-CE/koishi/tree/main/packages/web/app)) and every plugin share a single instance via externals and never end up with duplicate Vue copies.
