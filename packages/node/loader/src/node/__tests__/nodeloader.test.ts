...
async function withDir(fn: (dir: string) => Promise<void>) {
	const dir = await fs.mkdtemp(
		join(tmpdir(), "koishi-loader-node-"),
	);
	try {
		await fn(dir);
	} finally {
...
describe("NodeLoader.init", () => {
	...
	it("显式传入配置文件时以其所在目录为 baseDir", async () => {
		await withDir(async (dir) => {
			...
			await Bun.write(
				join(dir, "instance", "koishi.config.json"),
				"{}",
			);
			const loader = new NodeLoader();
			loader.baseDir = dir;
...
describe("NodeLoader.readConfig", () => {
	...
	it("注入 env 文件、执行迁移并回写规范化配置", async () => {
		await withDir(async (dir) => {
			...
			expect(process.env["NLO_TEST_NAME"]).toBe(
				"from-local",
			);
...
describe("NodeLoader.migrateEntry", () => {
	it("数据库插件补默认数据库名 / 路径", () => {
		const loader = new NodeLoader();
		expect(
			loader.migrateEntry("database-mysql", undefined),
		).toEqual({
			database: "koishi",
		});
		expect(
			loader.migrateEntry("database-mongo", {}),
		).toEqual({
			database: "koishi",
		});
		// 已有配置不覆盖
		expect(
			loader.migrateEntry("database-postgres", {
				database: "custom",
			}),
		).toEqual({ database: "custom" });
		expect(
			loader.migrateEntry("database-sqlite", undefined),
		).toEqual({
			path: "data/koishi.db",
		});
	});
	...
	it("其余名字交给基类处理（非 group 返回 undefined）", () => {
		const loader = new NodeLoader();
		expect(
			loader.migrateEntry("other", undefined),
		).toBeUndefined();
	});
...
describe("NodeLoader.import", () => {
	...
	it("相对路径加载本地模块并缓存解析结果", async () => {
		await withDir(async (dir) => {
			...
			expect(await loader.import("./plugin-a.js")).toBe(
				mod,
			);
		});
	});
