...
describe("pluginCandidates", () => {
	...
	it("已带社区前缀的完整包名直接采用", () => {
		expect(
			pluginCandidates("koishi-plugin-foo", "/base"),
		).toEqual(["koishi-plugin-foo"]);
	});

	it("已带官方组织前缀的完整包名直接采用", () => {
		expect(
			pluginCandidates("@koishi-ce/plugin-foo", "/base"),
		).toEqual(["@koishi-ce/plugin-foo"]);
	});
	...
	it("scoped 名为内层补全社区前缀", () => {
		...
		expect(
			pluginCandidates("@scope/koishi-plugin-foo", "/base"),
		).toEqual(["@scope/koishi-plugin-foo"]);
	});

	it("仅组织名（无内层）抛错", () => {
		expect(() =>
			pluginCandidates("@broken", "/base"),
		).toThrow('cannot resolve plugin "@broken"');
	});

	it("相对路径相对 baseDir 解析为绝对路径", () => {
		expect(
			pluginCandidates("./local", "/base/dir"),
		).toEqual([resolve("/base/dir", "./local")]);
	});

	it("绝对路径直接采用", () => {
		expect(
			pluginCandidates(resolve("/x/y"), "/base"),
		).toEqual([resolve("/x/y")]);
	});
});
...
describe("resolvePlugin", () => {
	it("从 baseDir 的 node_modules 解析社区包入口", async () => {
		const dir = await fs.mkdtemp(
			join(tmpdir(), "koishi-loader-res-"),
		);
		try {
			await fs.mkdir(
				join(dir, "node_modules", "koishi-plugin-foo"),
				{
					recursive: true,
				},
			);
	...
	it("全部候选失败时抛错", () => {
		expect(() =>
			resolvePlugin("definitely-not-a-plugin", "/"),
		).toThrow(
...
describe("resolvePlugin 纯 fs 兜底（Bun 父目录快照缓存）", () => {
	...
	async function withDir(
		...
		const dir = await fs.mkdtemp(
			join(tmpdir(), "koishi-loader-fsres-"),
		);
		try {
			await fn(dir);
		} finally {
	...
	it("市场安装时序：装包前解析失败触发快照后，落盘包由 fs 兜底命中", async () => {
		await withDir(async (dir) => {
			...
			// 也找不到），同时把目录快照写入解析缓存
			expect(() =>
				resolvePlugin("late-install", dir),
			).toThrow('cannot resolve plugin "late-install"');
			...
			const pkgDir = await writePlugin(
				join(
					dir,
					"node_modules",
					"koishi-plugin-late-install",
				),
				{
					name: "koishi-plugin-late-install",
					main: "lib/index.cjs",
				},
	...
	it("作用域包 + scope 目录预存在（生产 @koishijs 场景）同样兜底命中", async () => {
		await withDir(async (dir) => {
			...
			await fs.mkdir(
				join(dir, "node_modules", "@koishijs"),
				{
					recursive: true,
				},
			);
			expect(() =>
				resolvePlugin("scoped-late", dir),
			).toThrow();
			...
			const pkgDir = await writePlugin(
				...
				{
					name: "@koishijs/plugin-scoped-late",
					main: "index.js",
				},
				{ "index.js": "module.exports = 1" },
			);
			expect(resolvePlugin("scoped-late", dir)).toBe(
	...
	it("exports 条件表按 bun → require → node → default 序取主入口", async () => {
		await withDir(async (dir) => {
			...
			expect(resolvePlugin("exp-demo", dir)).toBe(
				join(pkgDir, "lib/bun.mjs"),
			);
		});
	});
		...
		await withDir(async (dir) => {
			...
			const pkgDir = await writePlugin(
				join(
					dir,
					"node_modules",
					"koishi-plugin-nomain-demo",
				),
				{
					name: "koishi-plugin-nomain-demo",
					module: "lib/esm.mjs",
				},
	...
	it("包存在但入口不可识别 / 缺失时仍抛错", async () => {
		await withDir(async (dir) => {
			...
			expect(() =>
				resolvePlugin("broken-entry", dir),
			).toThrow();
			...
			await writePlugin(
				join(
					dir,
					"node_modules",
					"koishi-plugin-broken-entry",
				),
				{
					name: "koishi-plugin-broken-entry",
					main: "lib/missing.cjs",
				},
				{},
			);
			expect(() =>
				resolvePlugin("broken-entry", dir),
			).toThrow('cannot resolve plugin "broken-entry"');
		});
	});
});
