...
import { type Awaitable, isNullable, makeArray } from "cosmokit";
...
function collectFields<T extends keyof Tables>(
	argv: Argv,
...
class KoishiSession<U, G, C> {
	...
	async observeUser<T extends User.Field = never>(
		fields: Iterable<T>,
	): Promise<User.Observed<T | U>> {
