...
import { camelCase, type Dict, paramCase } from "cosmokit";
...
export namespace Argv {
	...
	export namespace whitespace {
		export const unescape = (source: string) =>
			source
		...
		export const escape = (source: string) =>
			source
	...
	export class CommandBase<T extends CommandBase.Config = CommandBase.Config> {
		...
		private _assignOption(
			...
			optionMap: OptionDeclarationMap,
		) {
		...
		parse(argv: string | Argv, terminator?: string): Argv {
			...
			while (!argv.error && argv.tokens?.length) {
				...
				if (
					content[0] !== "-" &&
					this.ctx.$commander.resolveDomain(argDecl.type).greedy
				) {
					args.push(
						this.ctx.$commander.parseValue(
							Argv.stringify(argv),
							"argument",
							argv,
							argDecl,
				...
				if (!quoted && (option = this._symbolicOptions[content])) {
					...
					if (
						...
						quoted ||
						(+content * 0 === 0 &&
							this.ctx.$commander.resolveDomain(argDecl.type).numeric)
					) {
						args.push(
							this.ctx.$commander.parseValue(
								content,
				...
				if (!param) {
					...
					if (this.ctx.$commander.resolveDomain(type).greedy) {
						...
						const isValued =
							names[names.length - 1] in (values || {}) || type === "boolean";
						if (
							!isValued &&
							argv.tokens.length &&
				...
				for (let j = 0; j < names.length; j++) {
					...
					if (optDecl && name in optDecl.values) {
						...
						options[key] = this.ctx.$commander.parseValue(
							source,
							"option",
							argv,
							optDecl,
