...
function chain(ctx: Context, ...args: unknown[]) {
	return (ctx.chain as (...args: unknown[]) => unknown)(
		...args,
	);
}
...
describe("Legacy Events", () => {
	it("waterfall 无监听器时原样返回首参", async () => {
		await expect(
			waterfall(app, "legacy-event-none", 42),
		).resolves.toBe(42);
	});
	...
	it("waterfall 依次传递上游返回值", async () => {
		...
		// 1 -> 2 -> 4
		await expect(
			waterfall(app, "legacy-waterfall", 1),
		).resolves.toBe(4);
	});
	...
	it("waterfall 支持以对象作为 thisArg", async () => {
		...
		await expect(
			waterfall(app, thisArg, "legacy-this-arg", 5),
		).resolves.toBe(15);
	});

	it("chain 为同步瀑布调用", () => {
		app.on(
			"legacy-chain" as never,
			((value: string) => `${value}a`) as never,
		);
		app.on(
			"legacy-chain" as never,
			((value: string) => `${value}b`) as never,
		);
