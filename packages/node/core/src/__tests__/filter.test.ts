...
describe("Selector API", () => {
	...
	it("context intersection", () => {
		...
		expect(app.guild("456", "789").guild("123", "456").filter(session1)).to.be
			.true;
		expect(app.guild("456", "789").guild("123", "789").filter(session1)).to.be
			.false;
		expect(app.guild("123", "789").guild("123", "456").filter(session1)).to.be
			.false;
		expect(app.user("123", "789").user("123", "456").filter(session1)).to.be
			.true;
		expect(app.user("456", "789").user("123", "456").filter(session1)).to.be
			.false;
		expect(app.user("123", "789").user("456", "789").filter(session1)).to.be
			.false;
	});
});
