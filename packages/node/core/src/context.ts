...
import { defineProperty, type Promisify, Time } from "cosmokit";
...
import SessionMixin, { type Session } from "./session";
...
export class Context extends satori.Context {
	...
	async waterfall(...args: [any, ...any[]]) {
		...
		for (const hook of this.lifecycle.filterHooks(
			this.lifecycle._hooks[name] || [],
			thisArg,
		)) {
