...
function withAuthority(authority: number) {
	return { user: { authority } as User.Observed<"authority"> };
}

describe("Permissions", () => {
	it("authority:N 用户等级判定", async () => {
		await expect(
			app.permissions.test("authority:1", session(withAuthority(2))),
		).resolves.toBe(true);
		await expect(
			app.permissions.test("authority:5", session(withAuthority(2))),
		).resolves.toBe(false);
		// 未登录（无 user 记录）时同样放行
		await expect(app.permissions.test("authority:1", session())).resolves.toBe(
			true,
		);
	});
	...
	it("define / provide 自定义权限", async () => {
		...
		await expect(
			app.permissions.test("vip", session({ userId: "123" })),
		).resolves.toBe(true);
		await expect(
			app.permissions.test("vip", session({ userId: "456" })),
		).resolves.toBe(false);
	});
	...
	it("inherit 满足任一上游权限即通过", async () => {
		...
		await expect(
			app.permissions.test("admin", session(withAuthority(2))),
		).resolves.toBe(true);
		await expect(
			app.permissions.test("admin", session(withAuthority(1))),
		).resolves.toBe(false);
	});
	...
	it("depend 依赖须全部满足", async () => {
		...
		await expect(
			app.permissions.test("root", session(withAuthority(2))),
		).resolves.toBe(false);
	});
	...
	it("本地授权列表兜底（会话 > 用户 > 频道）", async () => {
		await expect(
			app.permissions.test("perm.x", session({ permissions: ["perm.x"] })),
		).resolves.toBe(true);
		await expect(
			app.permissions.test(
				"perm.x",
				session({ user: { permissions: ["perm.x"] } as never }),
			),
		).resolves.toBe(true);
		await expect(
			app.permissions.test(
				"perm.x",
				session({ channel: { permissions: ["perm.x"] } as never }),
			),
		).resolves.toBe(true);
		await expect(app.permissions.test("perm.x", session())).resolves.toBe(
			false,
		);
	});
	...
	it("shadow 会话还原为原始会话再校验", async () => {
		...
		await expect(app.permissions.test("authority:2", shadow)).resolves.toBe(
			true,
		);
	});
