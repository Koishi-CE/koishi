...
const matchI18n = app.match as (
	pattern: string,
	response: (session: Session, params: [string, ...string[]]) => string,
	options: { i18n?: boolean; regex?: boolean; fuzzy?: boolean },
) => unknown;

// 正则模式：exec 捕获组作为回复参数
app.match(/^re-(\d+)$/, (_session, captured) => `编号${captured[1]}`);
...
matchI18n("short.num", (_session, captured) => `fuzzy:${captured[1] ?? ""}`, {
	i18n: true,
	regex: true,
	fuzzy: true,
});
