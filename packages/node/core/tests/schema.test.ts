...
describe("Schema Factories", () => {
	it("Schema.dynamic 打上 dynamic 角色并携带名称", () => {
		...
		expect(schema.meta.extra).toHaveShape({
			name: "my-plugin",
		});
	});
	...
	it("Schema.computed 包装内层为计算属性", () => {
		...
		const input = {
			$switch: { branches: [] },
		} as unknown as Parameters<typeof schema>[0];
		expect(schema(input)).toHaveShape({
			$switch: { branches: [] },
		});
	});
...
describe("Schema Service", () => {
	...
	it("extend / set 的内容随定义方上下文销毁移除", () => {
		...
		expect(
			app.schema.get("test.dispose").list,
		).toHaveLength(1);
		expect(app.schema.get("test.override").type).toBe(
			"string",
		);
